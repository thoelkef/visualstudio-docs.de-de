...
Um einen Verstoß gegen diese Regel zu beheben, müssen Sie <xref:System.IDisposable.Dispose%2A> für Felder mit Typen, die <xref:System.IDisposable> implementieren, aufgerufen werden.
...
Der folgende Code Ausschnitt zeigt einen Typ `TypeB`, der gegen Regel CA2213 verstößt, indem er ein Feld `aFieldOfADisposableType` als verwerfbaren Typ (`TypeA`) deklariert und <xref:System.IDisposable.Dispose%2A> nicht für das Feld aufgerufen wird.
