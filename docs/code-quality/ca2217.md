...
Für eine Enumeration sollte <xref:System.FlagsAttribute> nur vorhanden sein, wenn jeder in der-Enumeration definierte Wert eine Potenz von zwei oder einer Kombination von definierten Werten ist.
...
Der folgende Code zeigt eine Enumeration, `Color`, die den Wert 3 enthält. 3 ist keine Potenz von zwei oder eine Kombination aus den definierten Werten. Die `Color`-Enumeration darf nicht mit <xref:System.FlagsAttribute> markiert werden.
