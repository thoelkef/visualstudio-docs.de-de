...
- die <xref:System.Xml.XmlReaderSettings.DtdProcessing%2A>-Eigenschaft ist auf "analysieren" festgelegt.

- Nicht vertrauenswürdige Eingaben werden mithilfe von <xref:System.Xml.XmlResolver> anstelle von <xref:System.Xml.XmlSecureResolver> verarbeitet.
...
- Verwenden Sie den <xref:System.Xml.XmlSecureResolver>, um die Ressourcen einzuschränken, auf die der XmlTextReader zugreifen kann.
