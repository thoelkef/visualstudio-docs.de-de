...
Die <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> Attribut ist ein conditional-Attribut der in der Assembly mit verwaltetem Code, den IL-Metadaten enthalten ist, nur dann, wenn das Symbol für die CODE_ANALYSIS-Kompilierung zum Zeitpunkt der Kompilierung definiert wird.
...
> [!NOTE]
> Wenn Sie ein Projekt in Visual Studio 2017 migrieren, können Sie plötzlich eine überwältigende Anzahl von Warnungen der Codeanalyse ausgesetzt sind. Wenn Sie nicht möchten Sie die Warnungen beheben und die Codeanalyse vorübergehend deaktivieren möchten, öffnen Sie die Eigenschaftenseiten des Projekts (**Projekt** > **\<Projekt > Eigenschaften**) und wechseln Sie zu die **Codeanalyse** Registerkarte. Deaktivieren Sie **Codeanalyse für Build aktivieren**, und klicken Sie dann Ihr Projekt neu erstellen. Alternativ können Sie eine andere, kleinere der Regeln für den Code ausführen auswählen. Denken Sie daran, aktivieren Sie die Codeanalyse auf Wenn Sie bereit, die Warnungen zu beheben sind.
...
- **Regel Kategorie** -die Kategorie, in dem die Regel definiert wird. Weitere Informationen zu Codeanalyseregeln, finden Sie unter [Managed Code (Warnungen)](../code-quality/code-analysis-for-managed-code-warnings.md).
