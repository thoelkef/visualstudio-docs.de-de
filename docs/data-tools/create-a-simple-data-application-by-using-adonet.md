---
title: "Exemplarische Vorgehensweise: Erstellen einer einfachen Datenanwendung mit ADO.NET | Microsoft Docs"
ms.custom: ""
ms.date: "12/16/2016"
...
-   [Die Formulare erstellen und Steuerelemente hinzufügen](../data-tools/create-a-simple-data-application-by-using-adonet.md#BKMK_createtheformsandaddcontrols)  
...
-   [Die Verbindungszeichenfolge abrufen](../data-tools/create-a-simple-data-application-by-using-adonet.md#BKMK_retrievetheconnectionstring)  
  
-   [Den Code für die Formulare schreiben](../data-tools/create-a-simple-data-application-by-using-adonet.md#BKMK_writethecodefortheforms)  
  
-   [Die Anwendung testen](../data-tools/create-a-simple-data-application-by-using-adonet.md#BKMK_testyourapplication)  
  
## Vorbereitungsmaßnahmen  
 Zum Erstellen der Anwendung benötigen Sie:  
  
-   Visual Studio 2012 mit Update 1 oder [!INCLUDE[vs_dev12](../data-tools/includes/vs_dev12_md.md)]  
  
-   SQL Server 2012 Express LocalDB  
  
-   Die kleine Beispieldatenbank, die Sie erstellen, indem Sie die unter [Exemplarische Vorgehensweise: Erstellen einer kleinen Beispieldatenbank](../data-tools/create-a-sql-database-by-using-a-script.md) beschriebenen Schritte befolgen.  
  
-   Die Verbindungszeichenfolge für die Datenbank, nachdem Sie sie eingerichtet haben.  Sie können diesen Wert suchen, indem Sie den **SQL Server\-Objekt\-Explorer** öffnen, das Kontextmenü für die Datenbank öffnen, **Eigenschaften** auswählen, und dann zur Eigenschaft **Verbindungszeichenfolge** wechseln.  
  
 In diesem Thema wird davon ausgegangen, dass Sie mit der grundlegenden Funktionalität der Visual Studio IDE vertraut sind und eine Windows Forms\-Anwendung erstellen können, diesem Projekt Formulare hinzufügen können, Schaltflächen und andere Steuerelemente in diesen Formularen einfügen können, Eigenschaften für diese Steuerelemente einrichten einfache Ereignisse programmieren können.  Wenn Sie nicht mit diesen Aufgaben vertraut sind, wird empfohlen, die [Erste Schritte mit Visual C\# und Visual Basic](../ide/getting-started-with-visual-csharp-and-visual-basic.md) abzuschließen, bevor Sie mit diesem Thema beginnen.  
  
##  <a name="BKMK_setupthesampledatabase"></a> Die Beispieldatenbank einrichten  
 Die Beispieldatenbank für diese exemplarische Vorgehensweise besteht aus den Tabellen "Customer" und "Orders".  Die Tabellen enthalten anfangs keine Daten, aber Sie fügen Daten hinzu, wenn Sie die von Ihnen erstellte Anwendung ausführen.  Die Datenbank hat auch fünf einfache gespeicherte Prozeduren.  [Exemplarische Vorgehensweise: Erstellen einer kleinen Beispieldatenbank](../data-tools/create-a-sql-database-by-using-a-script.md) enthält ein Transact\-SQL\-Skript, das die Tabellen, die Primär\- und Fremdschlüssel, die Einschränkungen und die gespeicherten Prozeduren erstellt.  
  
##  <a name="BKMK_createtheformsandaddcontrols"></a> Die Formulare erstellen und Steuerelemente hinzufügen  
...
##  <a name="BKMK_storetheconnectionstring"></a> Die Verbindungszeichenfolge speichern  
 Wenn die Anwendung versucht, eine Verbindung zur Datenbank zu öffnen, muss die Anwendung Zugriff auf die Verbindungszeichenfolge haben.  Um zu vermeiden, dass die Zeichenfolge auf jedem Formular manuell eingegeben werden muss, speichern Sie die Zeichenfolge in der Datei "App.config" im Projekt, und erstellen Sie eine Methode, die die Zeichenfolge zurückgibt, wenn sie über ein beliebiges Formular in der Anwendung aufgerufen wird.  
  
1.  Öffnen Sie das Kontextmenü für das Projekt, und wählen Sie dann **Eigenschaften** aus.  
  
2.  Im linken Bereich des Fensters **Eigenschaften** wählen Sie die Registerkarte **Einstellungen** aus.  
  
3.  In der Spalte **Name** geben Sie `connString` ein.  
  
4.  Wählen Sie in der Liste **Typ\(Verbindungszeichenfolge\)** aus.  
  
5.  Wählen Sie in der Liste **Bereich** den Eintrag **Anwendung** aus.  
  
6.  Geben Sie in der Spalte **Wert** die Verbindungszeichenfolge ein, und speichern Sie die Änderungen.  
  
##  <a name="BKMK_retrievetheconnectionstring"></a> Die Verbindungszeichenfolge abrufen  
  
1.  Wählen Sie auf der Menüleiste **Projekt** und **Verweis hinzufügen** aus, und fügen Sie dann einen Verweis auf "System.Configuration.dll" hinzu.  
  
2.  Wählen Sie in der Menüleiste **Projekt**, **Klasse hinzufügen** aus, um dem Projekt eine Klassendatei hinzuzufügen, und nennen Sie die Datei `Utility`.  
  
     Visual Studio erstellt die Datei und zeigt sie im **Projektmappen\-Explorer** an.  
...
3.  Ersetzen Sie den Platzhaltercode in der Datei "Utility" durch folgenden Code.  Beachten Sie die nummerierten Kommentare \(mit vorangestelltem "Uti\-"\), die die Abschnitte des Codes kennzeichnen.  In der Tabelle, die unter dem Code steht, werden wesentliche Punkte aufgeführt.  
    ...
    |Util\-3|Obwohl Sie `connString` als Namen der Verbindungszeichenfolge im Fenster **Eigenschaften** eingegeben haben, müssen Sie `"SimpleDataApp.Properties.Settings.connString"` \(C\#\) oder `"SimpleDataApp.My.MySettings.connString"` \(Visual Basic\) im Code angeben.|  
  
##  <a name="BKMK_writethecodefortheforms"></a> Den Code für die Formulare schreiben  
...
#### Ereignishandler erstellen  
 Erstellen Sie leere Click\-Ereignishandler für die drei Schaltflächen in dem Formular.  Siehe [Gewusst wie: Erstellen von Standardereignishandlern im Windows Forms\-Designer](http://msdn.microsoft.com/de-de/757bcc16-1dc2-4d68-b115-ac0f53f05c8d).  
  
#### Code für "Navigation" erstellen  
...
|Kommentar|Beschreibung|  
|---------------|------------------|  
|NC\-1|Fügen Sie System.Data.SqlClient und System.Configuration zur Liste der Namespaces hinzu.|  
...
|NC\-7|Erstellen Sie das `SqlCommand`\-Objekt `cmdNewCustomer`.<br /><br /> -   Geben Sie `Sales.uspNewCustomer` als die auszuführende gespeicherte Prozedur an.<br />-   Geben Sie anhand der Eigenschaft `CommandType` an, dass der Befehl eine gespeicherte Prozedur ist.|  
|NC\-8|Fügen Sie den Eingabeparameter `@CustomerName` aus der gespeicherten Prozedur ein.<br /><br /> -   Fügen Sie den Parameter der Auflistung `Parameters` hinzu.<br />-   Verwenden Sie die SqlDbType\-Enumeration, um den Parametertyp als "nvarchar \(40\)" anzugeben.<br />-   Geben Sie `txtCustomerName.Text` als Quelle an.|  
|NC\-9|Fügen Sie den Ausgabeparameter aus der gespeicherten Prozedur ein.<br /><br /> -   Fügen Sie den Parameter der Auflistung `Parameters` hinzu.<br />-   Verwenden Sie `ParameterDirection.Output`, um den Parameter als Ausgabe zu identifizieren.|  
|NC\-10|Fügen Sie einen Try\/Catch\/Finally\-Block hinzu, um die Verbindung zu öffnen, führen Sie die gespeicherte Prozedur aus, behandeln Sie Ausnahmen, und schließen Sie dann die Verbindung.|  
|NC\-11|Öffnen Sie die Verbindung \(`conn`\), die Sie unter NC\-6 erstellt haben.|  
|NC\-12|Verwenden Sie die `cmdNewCustomer`\-Methode von `ExecuteNonQuery`, um die gespeicherte Prozedur `Sales.uspNewCustomer` auszuführen, die eine `INSERT`\-Anweisung ausführt, keine Abfrage.|  
|NC\-13|Der `@CustomerID`\-Wert wird als Identitätswert aus der Datenbank zurückgegeben.  Da es eine ganze Zahl ist, müssen Sie sie in eine Zeichenfolge konvertieren, um sie im Textfeld für die Kunden\-Id anzuzeigen.<br /><br /> -   Sie haben `parsedCustomerID` unter NC\-2 deklariert.<br />-   Speichern Sie den Wert `@CustomerID` in `parsedCustomerID` für die spätere Verwendung.<br />-   Konvertieren Sie die zurückgegebene Kunden\-ID in eine Zeichenfolge, und fügen Sie sie in `txtCustomerID.Text` ein.|  
|NC\-14|Fügen Sie für dieses Beispiel eine einfache catch\-Klausel hinzu, die keine Produktionsqualität hat.|  
|NC\-15|Schließen Sie eine Verbindung immer, wenn Sie sie nicht mehr verwenden, sodass sie für den Verbindungspool freigegeben werden kann.  Siehe [SQL Server\-Verbindungspooling \(ADO.NET\)](http://msdn.microsoft.com/library/8xx3tyca\(l=en-us,v=VS.110\).aspx).|  
|NC\-16|Definieren Sie eine Methode, um zu überprüfen, ob ein Kundenname vorhanden ist.<br /><br /> -   Wenn das Textfeld leer ist, sollte eine Meldung angezeigt und `false` zurückgegeben werden, da ein Name für die Kontoerstellung erforderlich ist.<br />-   Wenn das Textfeld nicht leer ist, sollte `true` zurückgegeben werden.|  
...
### FillOrCancel\-Formular  
 Das FillorCancel\-Formular führt eine Abfrage aus, um einen Auftrag zurückzugeben, wenn Sie eine Auftrags\-ID eingeben und die Schaltfläche **Auftrag suchen** auswählen.  Die zurückgegebene Zeile wird in einem schreibgeschützten Datenraster angezeigt.  Sie können den Auftrag als abgebrochen markieren \(X\), wenn Sie auf die Schaltfläche **Auftrag abbrechen** klicken, oder Sie können den Auftrag als erfüllt markieren \(F\), wenn Sie auf die Schaltfläche **Auftrag ausfüllen** klicken.  Wenn Sie noch einmal auf die Schaltfläche **Auftrag suchen** klicken, wird die aktualisierte Zeile angezeigt.  
  
#### Ereignishandler erstellen  
 Erstellen Sie leere Click\-Ereignishandler für die vier Schaltflächen in dem Formular.  
  
#### Erstellen Sie Code für "FillOrCancel".  
...
|Kommentar|Beschreibung|  
|---------------|------------------|  
|FC\-1|Fügen Sie der Liste von Namespaces System.Data.SqlClient, System.Configuration und System.Text.RegularExpressions hinzu.|  
...
|FC\-5|Kommt Ihnen das bekannt vor?  Diese Aufgaben sind erforderlich, bevor Sie versuchen, eine SQL\-Anweisung oder eine gespeicherte Prozedur auszuführen.<br /><br /> -   Erstellen Sie ein SqlConnection\-Objekt.<br />-   Definieren Sie die SQL\-Anweisung, oder geben Sie den Namen der gespeicherten Prozedur an.  \(In diesem Fall führen Sie eine `SELECT`\-Anweisung aus.\)<br />-   Erstellen eines `SqlCommand`\-Objekts<br />-   Definieren Sie alle Parameter für die SQL\-Anweisung oder die gespeicherte Prozedur.|  
|FC\-6|Dieser Code verwendet `SqlDataReader` und `DataTable`, um das Abfrageergebnis abzurufen und anzuzeigen.<br /><br /> -   Öffnen Sie die Verbindung.<br />-   Erstellen Sie den SqlDataReader `rdr`, indem Sie die `cmdOrderID`\-Methode von `ExecuteReader` ausführen.<br />-   Erstellen Sie ein `DataTable`\-Objekt, um die abgerufenen Daten aufzunehmen.<br />-   Laden Sie die Daten aus `SqlDataReader` in das `DataTable`\-Objekt.<br />-   Zeigen Sie die Daten in der datagridview an, indem Sie `DataTable` als `DataSource` für die datagridview angeben.<br />-   Schließen Sie SqlDataReader.|  
...
|FC\-9|Erstellen Sie eine Methode, um sicherzustellen, dass `OrderID` bereit ist, als Parameter an das `SqlCommand`\-Objekt übergeben zu werden.<br /><br /> -   Überprüfen Sie, ob eine ID in `txtOrderID` eingegeben wurde.<br />-   Verwenden Sie `Regex.IsMatch`, um eine einfache Überprüfung für nicht ganzzahlige Zeichen zu definieren.<br />-   Sie haben die `parsedOrderID`\-Variable unter FC\-2 deklariert.<br />-   Wenn die Eingabe gültig ist, konvertieren Sie den Text in eine ganze Zahl, und speichern Sie den Wert in der `parsedOrderID`\-Variablen.<br />-   Umschließen Sie die Click\-Ereignishandler `isOrderID`, `btnFindByOrderID` und `btnCancelOrder` mit der `btnFillOrder`\-Methode.|  
  
##  <a name="BKMK_testyourapplication"></a> Die Anwendung testen  
 Drücken Sie die Taste F5, um die Anwendung zu erstellen und zu testen, nachdem Sie die einzelnen Click\-Ereignishandler programmiert haben und nachdem Sie die Programmierung beendet haben.