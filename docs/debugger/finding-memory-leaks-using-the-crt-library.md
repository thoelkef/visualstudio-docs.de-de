...
helpviewer_keywords: 
  ...
  - "_CrtSetDbgFlag"
  - "_CrtSetReportMode"
  - "Haltepunkte, Für Speicherbelegung"
  - "Debuggen von Speicherverlusten"
  - "Speicherverluste"
  - "Speicherverluste, Entdecken und Isolieren"
  - "Speicher, Debuggen"
...
```  
 ...
 Durch Verwendung von "crtdbg.h" werden die `malloc`\- und [free](/visual-cpp/c-runtime-library/reference/free)\-Funktionen den entsprechenden Debugversionen [\_malloc\_dbg](/visual-cpp/c-runtime-library/reference/malloc-dbg) und `free` zugeordnet, die die Speicherbelegung und \-freigabe nachverfolgen. Diese Zuordnung findet nur in Debugbuilds mit `_DEBUG` statt. Releasebuilds verwenden die normalen Funktionen `malloc` und `free`.  
...
## Interpretieren des Arbeitsspeicherverlust\-Berichts  
 Wenn `_CRTDBG_MAP_ALLOC` nicht in der Anwendung definiert ist, zeigt [\_CrtDumpMemoryLeaks](/visual-cpp/c-runtime-library/reference/crtdumpmemoryleaks) einen Arbeitsspeicherverlust\-Bericht an, der wie folgt aussieht:  
...
-   Der [Blocktyp](http://msdn.microsoft.com/de-de/e2f42faf-0687-49e7-aa1f-916038354f97) \(in diesem Beispiel `normal`\)  
...
-   Die ersten 16 Bytes Daten im Block im Hexadezimalformat  
 ...
 Diese Methoden funktionieren für Speicher, der mithilfe der CRT\-Standardfunktion `malloc` belegt wird. Belegt das Programm Speicher mithilfe des C\+\+\-Operators `new`, müssen Sie jedoch `new` neu definieren, wenn die Datei und die Zeilennummern im Arbeitsspeicherverlust\-Bericht angezeigt werden sollen. Dazu können Sie einen Codeblock wie den folgenden verwenden:  
  
```  
#ifdef _DEBUG #ifndef DBG_NEW #define DBG_NEW new ( _NORMAL_BLOCK , __FILE__ , __LINE__ ) #define new DBG_NEW #endif #endif  // _DEBUG  
```  
  
## Festlegen von Haltepunkten für eine Speicherbelegungsnummer  
...
3.  Geben Sie im **Überwachungsfenster**`_crtBreakAlloc` in der Spalte **Name** ein.  
...
## Vergleichen von Arbeitsspeicherzuständen  
