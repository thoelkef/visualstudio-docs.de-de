---
title: Übersicht über die Grafik Diagnose | Microsoft-Dokumentation
ms.custom: seodec18
ms.date: 02/09/2017
ms.topic: conceptual
author: mikejo5000
...
## <a name="directx-version-compatibility"></a>DirectX-Versionskompatibilität
 Grafikdiagnose unterstützt apps, die Direct3D 10 oder höher verwenden, und bietet eingeschränkte Unterstützung für apps, die Direct2D verwenden. Sie unterstützt keine Apps, die frühere Versionen von Direct3D und DirectDraw verwenden oder andere Grafik-APIs.

### <a name="windows-10-and-direct3d-12"></a>Windows 10 und Direct3D 12
 Windows 10 hat *Direct3D 12*eingeführt, was sich wesentlich von Direct3D 10 und Direct3D 11 unterscheidet. Diese Unterschiede bringen DirectX wieder in Einklang mit moderner Grafikhardware und ermöglichen die Nutzung des gesamten Potenzials der Software. Sie bringen jedoch auch umfangreiche API-Änderungen mit sich und übertragen mehr Verantwortung auf den Programmierer, wenn es um die Verwaltung der Lebensdauer von Ressourcen sowie die Behandlung von Konflikten geht. Trotz der Unterschiede behält Grafikdiagnose mit Direct3D 12 die Funktions Parität mit Grafikdiagnose mit Direct3D 11,2 bei.

 Windows 10 bietet außerdem weiterhin Unterstützung für ältere Versionen von Direct3D sowie für die Spiele und Anwendungen, die davon abhängig sind. Grafikdiagnose in Visual Studio unterstützt weiterhin Direct3D 10 und Direct3D 11 unter Windows 10.
...
## <a name="graphics-analyzer"></a>Grafikanalyse
 ...
 ![Alle Grafik Debugger-Fenster](media/graphicsdebuggerwindows.png "Graphicsdebuggerwindows")
...
### <a name="the-graphics-toolbar-graphics-analyzer"></a>Die Grafiksymbolleiste (Grafikanalyse)
 ...
 ![Grafik Symbolleiste im Grafik Diagnosemodus](media/vsg_toolbar.png "vsg_toolbar")
...
### <a name="pipeline-stages"></a>Pipelinestufen
 ...
 Im Fenster " [Pipeline Stufen](graphics-pipeline-stages.md) " wird das Ergebnis jeder Phase unabhängig visualisiert, sodass Sie leichter bestimmen können, in welcher Phase ein Renderingproblem zuerst auftritt. Sobald Sie die Stufe ermittelt haben, können Sie mit dem Debuggen des Shaders direkt im Fenster „Pipelinestufen“ beginnen.
...
### <a name="pixel-history"></a>Pixelverlauf
 ...
 Das Fenster [Pixel Verlauf](graphics-pixel-history.md) visualisiert den gesamten Shader-Verlauf jedes Pixels im Frame, einschließlich abgelehnter Beiträge. Für Beiträge, die nicht abgelehnt wurden, zeigt der Grafikpixelverlauf die unformatierten Shaderergebnisse sowie Informationen darüber an, wie jede neue Farbe mit der vorherigen kombiniert wurde. Mit diesen Informationen ist es viel einfacher, die Fehlerursache in Pixeln zu ermitteln, die Shaderergebnisse kombinieren, oder wenn ein gerendertes Objekt fehlt, da sein Beitrag zum Pixel fälschlicherweise abgelehnt wurde.
...
### <a name="event-call-stack"></a>Ereignisaufrufliste
 ...
 Im Fenster [Ereignis-Aufrufstapel](graphics-event-call-stack.md) wird die komplette Aufrufliste aller Grafik Ereignisse in der Ereignisliste angezeigt, und Sie können auch zum Quellcode Ihrer APP springen, wenn Debuginformationen verfügbar sind. Dies ist ein leistungsfähiges Tool, um einen Fehler bei seinem ersten Auftreten in der GPU bis zurück zu seinem Ursprung im Quellcode Ihrer App zu verfolgen.
...
### <a name="object-table"></a>Objekttabelle
 ...
 Die [Objekttabelle](graphics-object-table.md) zeigt alle Objekte an, die zum Zeitpunkt des in der Ereignisliste ausgewählten Grafik Ereignisses vorhanden sind. Da die meisten Objekte in einer typischen App Texturen sind, ist die Ereignisliste so optimiert, dass Sie die für Bilder relevanten Details auf einen Blick erkennen können. Die Spalte „Typ“ gibt an, welche Art von Objekt in jeder Zeile aufgeführt ist, und die Spalte „Format“ zeigt darüber hinaus den Untertyp oder die Version des Objekts an. Weitere Details werden ebenfalls angezeigt. Einige Objekte verfügen außerdem über Hyperlinks, denen Sie folgen können, um das Objekt mit einem speziellen Viewer anzuzeigen. So können Sie zum Beispiel Texturen als Bilder anzeigen, oder für Puffer können Sie durch Definition des Pufferformats festlegen, wie der Pufferviewer die umformatierten Pufferbytes analysieren und anzeigen soll.
