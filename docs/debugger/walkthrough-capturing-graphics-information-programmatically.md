---
title: "Exemplarische Vorgehensweise: Programmgesteuertes Erfassen von Grafikinformationen | Microsoft Docs"
ms.custom: ""
ms.date: "12/16/2016"
...
##  <a name="CaptureDX11_2"></a> Programmgesteuerte Erfassung in Windows 8.1  
 In diesem Teil der exemplarischen Vorgehensweise wird die programmgesteuerte Erfassung in Apps gezeigt, die die DirectX 11.2\-API unter Windows 8.1 verwenden, in der die Methode der stabilen Erfassung verwendet wird. Informationen zur Verwendung der programmgesteuerten Erfassung in Apps, die frühere Versionen von DirectX unter Windows 8.0 verwenden, finden Sie unter [Programmgesteuerte Erfassung in Windows 8.0 und früher](#CaptureDX11_1) weiter unten in diesem Artikel.  
...
-   Schließen Sie diese Header in die Quelldatei ein, in der Sie die IDXGraphicsAnalysis\-Schnittstelle definieren:  
    ...
    > [!NOTE]
    >  Wenn das DirectX SDK vom Juni 2010 auf Ihrem Computer installiert wurde und der Include\-Pfad Ihres Projekts `%DXSDK_DIR%include\x86` enthält, verschieben Sie diesen Teil an das Ende des Include\-Pfads. Gehen Sie beim Bibliothekspfad genauso vor.  
  
#### Windows Phone 8.1  
 Da das Windows Phone 8.1 SDK den `DXProgrammableCapture.h`\-Header nicht enthält, müssen Sie die `IDXGraphicsAnalysis`\-Schnittstelle selbst definieren, damit Sie die `BeginCapture()`\- und die `EndCapture()`\-Methode verwenden können. Schließen Sie die anderen Header so ein, wie dies im vorherigen Abschnitt beschrieben ist.  
...
> [!IMPORTANT]
>  Wenn Sie programmgesteuerte Erfassung verwenden, müssen Sie Ihre App weiterhin unter der Grafikdiagnose ausführen \(Alt \+ F5 in [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]\).  
...
-   Verwenden Sie den folgenden Code, um die IDXGraphicsAnalysis\-Schnittstelle an die DXGI\-Debugschnittstelle anzuschließen.  
    ...
    > [!NOTE]
    >  Wenn `DXGIGetDebugInterface1` den Wert `E_NOINTERFACE` zurückgibt \(`Fehler: E_NOINTERFACE Schnittstelle nicht unterstützt`\), müssen Sie sicherstellen, dass die App unter der Grafikdiagnose \(Alt\+F5 in [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]\) ausgeführt wird.  
...
##  <a name="CaptureDX11_1"></a> Programmgesteuerte Erfassung in Windows 8.0 und früher  
