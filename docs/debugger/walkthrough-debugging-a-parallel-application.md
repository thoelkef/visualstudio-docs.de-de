...
1.  Auf der **Debuggen** Sie im Menü **Windows** , und klicken Sie dann auf **parallele Stapel**. Stellen Sie sicher, dass **Threads** in das Feld in der oberen linken Ecke ausgewählt ist.  
     ...
     Die Aufrufliste des Hauptthreads wird in einem Feld angezeigt, während die Aufruflisten für die anderen vier Threads als Gruppe in einem anderen Feld angezeigt werden. Vier Threads werden als Gruppe angezeigt, da für ihre Stapelrahmen dieselben Methodenkontexte verwendet werden; d. h., sie befinden sich in denselben Methoden: `A`, `B` und `C`. Ansicht der Thread-IDs und Namen der Threads, die gemeinsam das gleiche Feld zeigen Sie auf den Header (**4 Threads**). Der aktuelle Thread wird fett formatiert angezeigt, wie in der folgenden Abbildung veranschaulicht.  
     ...
     ![QuickInfo auf aktivem Stapelrahmen](../debugger/media/pdb_walkthrough_1b.png "PDB_Walkthrough_1B")  
...
1.  Um die Ausführung fortzusetzen, bis zum zweite Haltepunkt erreicht wird, auf die **Debuggen** Menü klicken Sie auf **Fortfahren**. In der folgenden Abbildung wird die Threadstruktur beim zweiten Haltepunkt dargestellt.  
     ...
     Mit dem Faden-Symbol (einander überlappenden blauen und roten Wellenlinien) werden die aktiven Stapelrahmen der nicht aktuellen Threads angegeben. In der **Aufrufliste** Fenster, doppelklicken Sie auf S.B, um die Frames zu wechseln. Die **parallele Stapel** Fenster gibt den aktuellen Stapelrahmen des aktuellen Threads an, mit einem grünen gekrümmten Pfeilsymbol.  
     ...
     Bei großen Anwendungen mit einer Vielzahl von Threads kann es sich empfehlen, sich nur auf eine Teilmenge von Threads zu konzentrieren. Die **parallele Stapel** Fenster können Aufruflisten nur für gekennzeichnete Threads angezeigt. Klicken Sie auf der Symbolleiste auf die **nur gekennzeichnete Elemente anzeigen** Schaltfläche neben dem Listenfeld.  
  
     ![Leere Fenster "Parallele Stapel" und QuickInfo](../debugger/media/pdb_walkthrough_3a.png "PDB_Walkthrough_3A")  
  
     Im nächsten Schritt in der **Threads** Fenster Flag einzelnen threads, um festzustellen, wie in ihren Aufruflisten angezeigt werden die **parallele Stapel** Fenster. Verwenden Sie zum Kennzeichnen von Threads das Kontextmenü oder die erste Zelle eines Threads. Klicken Sie auf die **nur gekennzeichnete Elemente anzeigen** Symbolleisten-Schaltfläche erneut aus, um alle Threads angezeigt.  
...
1.  Um die Ausführung fortzusetzen, bis zum dritte Haltepunkt erreicht wird, auf die **Debuggen** Menü klicken Sie auf **Fortfahren**.  
     ...
     ![Methodenansicht im Fenster "Parallele Stapel"](../debugger/media/pdw_walkthrough_4.png "PDW_Walkthrough_4")  
    ...
    -   **Symbolinformationen laden** und **Symboleinstellungen** die entsprechenden Dialogfelder zu öffnen.  
    ...
    -   **Externen Code anzeigen** werden alle Frames angezeigt, auch wenn sie nicht im Benutzercode sind. Das Diagramm wird erweitert, um die zusätzlichen Frames aufzunehmen (die möglicherweise abgeblendet dargestellt werden, da keine Symbole dafür vorhanden sind).  
     ...
     Wenn Sie bei großen Diagrammen zum nächsten Haltepunkt wechseln, können Sie einen automatischen Bildlauf der Anzeige zum aktiven Stapelrahmen des aktuellen Threads ausführen lassen (d. h. des Threads, der den Haltepunkt zuerst erreicht hat). In der **parallele Stapel** Fenster, stellen Sie sicher, dass die **automatischen Bildlauf zu aktuellem Stapelrahmen durchführen** befindet sich auf der Symbolleiste auf.  
  
     ![Automatischer Bildlauf in das Fenster "Parallele Stapel"](../debugger/media/pdb_walkthrough_4a.png "PDB_Walkthrough_4A")  
...
1.  Um die Ausführung fortzusetzen, bis zum vierte Haltepunkt erreicht wird, auf die **Debuggen** Menü klicken Sie auf **Fortfahren**.  
     ...
     Die **Vogelperspektive** auch hilfreich bei großen Diagrammen in der **parallele Stapel** Fenster. Sehen Sie die **Vogelperspektive** durch Klicken auf die Schaltfläche zwischen den Bildlaufleisten in der unteren rechten Ecke des Fensters, wie in der folgenden Abbildung dargestellt.  
  
     ![Vogelperspektivbilder &#45; Auge, zeigen im Fenster "Parallele Stapel"](../debugger/media/pdb_walkthrough_5.png "PDB_Walkthrough_5")  
     ...
     ![Vergrößert Stapel im Fenster "Parallele Stapel"](../debugger/media/pdb_walkthrough_5a.png "PDB_Walkthrough_5A")  
...
5.  Auf der **Debuggen** Sie im Menü **Windows** , und klicken Sie dann auf **Parallele Aufgaben**. Die folgende Abbildung zeigt die **Aufgaben** Fenster.  
  
     ![Vier mit Aufgaben im Fenster "Aufgaben"](../debugger/media/pdw_walkthrough_6.png "PDW_Walkthrough_6")  
...
1.  Um die Ausführung fortzusetzen, bis zum zweite Haltepunkt erreicht wird, auf die **Debuggen** Menü klicken Sie auf **Fortfahren**.  
     ...
     Zuvor die **Status** Spalte alle Aufgaben als aktiv angezeigt, jedoch zwei Aufgaben sind nun wartend. Aufgaben können aus vielen anderen Gründen blockiert werden. In der **Status** Spalte, zeigen Sie auf eine wartende Aufgabe, um zu erfahren, warum er blockiert wird. In der folgenden Abbildung wartet beispielsweise Aufgabe 3 auf Aufgabe 4.  
     ...
     ![Threadansicht im Fenster "Parallele Stapel"](../debugger/media/pdb_walkthrough_8.png "PDB_Walkthrough_8")  
     ...
     ![Ansicht "Kontextmenü" im Fenster "Aufgaben"](../debugger/media/pdb_walkthrough_8a.png "PDB_Walkthrough_8A")  
...
1.  Um die Ausführung fortzusetzen, bis zum dritte Haltepunkt erreicht wird, auf die **Debuggen** Menü klicken Sie auf **Fortfahren**.  
     ...
     Um die über-/ unterordnungsbeziehung besser zu visualisieren, Maustaste die **übergeordneten** Spaltenüberschrift, und klicken Sie dann auf **übergeordneten untergeordnete Ansicht**. Die Anzeige entspricht der folgenden Abbildung:  
  
     ![Übergeordnete &#45; zeigen im Fenster "Aufgaben" untergeordnete](../debugger/media/pdb_walkthrough_9.png "PDB_Walkthrough_9")  
...
1.  Um die Ausführung fortzusetzen, bis zum dritte Haltepunkt erreicht wird, auf die **Debuggen** Menü klicken Sie auf **Fortfahren**. Klicken Sie auf die **ID** Spaltenüberschrift sortiert nach ID auf. Die Anzeige entspricht der folgenden Abbildung:  
     ...
     ![Gruppierte Stapel im Fenster "Parallele Stapel"](../debugger/media/pdb_walkthrough_12a.png "PDB_Walkthrough_12A")  
     ...
     !["Kontextmenü" im Fenster "Aufgaben"](../debugger/media/pdb_walkthrough_12b.png "PDB_Walkthrough_12B")  
