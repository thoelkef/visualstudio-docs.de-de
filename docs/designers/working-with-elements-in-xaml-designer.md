---
title: "Arbeiten mit Elementen im XAML-Designer | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
## Hinzufügen eines Elements zu einem Layout  
 *Layout* ist der Prozess der Größenanpassung und Positionierung von Elementen in einer Benutzeroberfläche.  Um visuelle Elemente zu positionieren, müssen Sie sie in einem Layout platzieren [Panel](http://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.panel.aspx).  Ein `Panel` verfügt über eine untergeordnete Eigenschaft, die eine Auflistung von [FrameworkElement](http://msdn.microsoft.com/library/windows/apps/br208706.aspx)\-Typen darstellt.  Sie können verschiedene untergeordnete `Panel`\-Elemente, wie z. B. [Canvas](http://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.canvas.aspx), [StackPanel](http://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.stackpanel.aspx) und [Grid](http://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.grid.aspx), als Layoutcontainer und zum Positionieren und Anordnen der Elemente auf einer Seite verwenden.  
...
#### So fügen Sie ein Element zu einem Layout hinzu  
...
-   Führen Sie im XAML\-Designer eine der folgenden Aktionen aus:  
    ...
    -   Ziehen Sie ein Element aus der **Toolbox** auf der Zeichenfläche.  
  
    -   Wählen Sie in der **Toolbox** eines der Zeichenwerkzeuge \(z. B. [Ellipse](http://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.shapes.ellipse.aspx) oder [Rechteck](http://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.shapes.rectangle.aspx)\) aus, und zeichnen Sie ein Element im aktiven Bereich.  
  
## Ändern der Ebenenreihenfolge von Elementen  
 Wenn auf der Zeichenfläche im XAML\-Designer zwei Elemente vorhanden sind, wird ein Element vor dem anderen in der Ebenenreihenfolge angezeigt.  Am unteren Rand der Liste der Elemente im Dokumentgliederungsfenster ist das vorderste Element \(außer wenn die **ZIndex**\-Eigenschaft für ein Element festgelegt ist\).  Wenn Sie ein Element in eine Seite, ein Formular oder einen Layoutcontainer einfügen, wird das Element automatisch vor anderen Elementen im aktiven Containerelement platziert.  Um die Reihenfolge der Elemente zu ändern, können Sie die **Reihenfolge**\-Befehle verwenden oder die Elemente in der Objektstruktur im Dokumentgliederungsfenster ziehen.  
...
-   Führen Sie einen der folgenden Schritte aus:  
    ...
    -   Klicken Sie mit der rechten Maustaste auf das Element im Dokumentgliederungsfenster oder auf der Zeichenfläche, für das bzw. die Sie die Ebenenreihenfolge ändern möchten, zeigen Sie auf **Reihenfolge**, und klicken Sie dann auf eine der folgenden Optionen:  
  
        -   **In den Vordergrund**, um das Element in der Reihenfolge ganz nach vorne zu holen.  
  
        -   **Eine Ebene nach vorne**, um das Objekt eine Ebene in der Reihenfolge nach vorne zu holen.  
  
        -   **Eine Ebene nach hinten**, um das Objekt eine Ebene in der Reihenfolge nach hinten zu bringen.  
  
        -   **In den Hintergrund**, um das Element hinter alle anderen Elemente in der Reihenfolge zu bringen.  
...
## Ändern der Ausrichtung eines Elements  
 ...
 Eine *Ausrichtungslinie* ist ein visueller Hinweis, der Ihnen hilft, ein Elements relativ zu anderen Elementen in der App auszurichten.  
...
2.  Wählen Sie eine der folgenden Eigenschaften unter **HorizontalAlignment** im **Layout**\-Abschnitt des Eigenschaftenfensters aus: **Links**, **Mitte**, **Rechts** oder **Strecken**.  
  
3.  Wählen Sie eine der folgenden Eigenschaften unter **VerticalAlignment** im **Layout**\-Abschnitt des Eigenschaftenfensters aus: **Oben**, **Mitte**, **Unten** oder **Strecken**.  
  
#### So richten Sie zwei oder mehr Elemente mithilfe von Ausrichtungslinien aus  
...
## Ändern der Ränder eines Elements  
...
2.  Ändern Sie im Eigenschaftenfenster unter **Layout** den Wert \(in Pixel oder in geräteunabhängigen Einheiten von etwa 1\/96 Zoll\) für die gewünschten **Ränder**\-Eigenschaften: **Top** \(Oben\), **Left** \(Links\), **Right** \(Rechts\) oder **Bottom** \(Unten\).  
...
-   Um die Ränder eines Elements relativ zu dessen Layoutcontainer zu ändern, klicken Sie auf die *Rand\-Adorner*, die um das Element auf der Zeichenfläche angezeigt werden, wenn das Element ausgewählt ist und sich in einem Container befindet.  Eine Abbildung der Rand\-Adorner finden Sie unter [Erstellen einer Benutzeroberfläche mit dem XAML\-Designer](../designers/creating-a-ui-by-using-xaml-designer-in-visual-studio.md).  
     ...
     Wenn Sie ein Rand\-Adorner öffnen und der gegenüberliegende Rand nicht festgelegt ist, wird der gegenüberliegende Rand auf den richtigen Wert gemäß der Position des Elements auf der Zeichenfläche festgelegt.  Für gegenüberliegende Ränder, wie z. B. der **linke** und der **rechte** Rand, ist immer mindestens eine Eigenschaft festgelegt.  
...
-   Klicken Sie mit der rechten Maustaste auf die Gruppierung, die Sie aufheben möchten, und klicken Sie auf **Gruppierung aufheben**.  
 ...
 Sie können Elemente auch durch Rechtsklicken auf ausgewählten Elemente im Dokumentgliederungsfenster und durch Klicken auf **Gruppieren in** oder **Gruppierung aufheben** gruppieren bzw. die Gruppierung aufheben.  
...
-   Klicken Sie im Dokumentgliederungsfenster oder auf der Zeichenfläche mit der rechten Maustaste auf das Element, wählen Sie **Layout**, **Zurücksetzen** *Eigenschaftsname* aus, wobei *Eigenschaftsname* die Eigenschaft ist, die Sie zurücksetzen möchten \(oder wählen Sie **Layout**, **Alle zurücksetzen** aus, um alle Layouteigenschaften für das Element zurückzusetzen\).  
