---
title: "Lokale Werte abrufen | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
ms.topic: "article"
helpviewer_keywords: 
  - "Auswertung von Ausdrücken, lokale Werte"
  - "Debuggen von lokalen Werte [Debugging-SDK]"
  - "Auswertung von Ausdrücken, die lokalen Werte abrufen"
...
>  In Visual Studio 2015 ist diese Art der Implementierung von ausdrucksauswertungen veraltet. Informationen über das Implementieren von CLR\-ausdrucksauswertungen finden Sie unter [CLR\-Ausdrucksauswertungen](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) und [Managed Expression Evaluator Sample](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample).  
 ...
 Um den Wert einer lokalen abzurufen, ruft Visual Studio [GetPropertyInfo](../../extensibility/debugger/reference/idebugproperty2-getpropertyinfo.md) für diese lokale. In dieser Implementierung der Klasse `CFieldProperty` implementiert die IDebugProperty2\-Schnittstelle für jede lokale.  
  
 Diese Implementierung der `IDebugProperty2::GetPropertyInfo` führt die folgenden Aufgaben:  
  
1.  Ruft des lokalen Namen, Eigenschaft und Attribute aus der [FIELD\_INFO](../../extensibility/debugger/reference/field-info.md) Struktur gefüllt, wenn die Klasse instanziiert und initialisiert wurde.  
  
2.  Ruft den lokalen Typ aus der [IDebugField](../../extensibility/debugger/reference/idebugfield.md) Objekt.  
  
3.  Ruft den lokalen Wert aus der `IDebugField` Objekt. Dieses Feld gebunden ist, auf die Speicheradresse des lokalen mithilfe der [IDebugBinder](../../extensibility/debugger/reference/idebugbinder.md) Objekts und der Wert stammt aus dem resultierenden [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) Objekt.  
  
4.  Gibt alle angeforderte Eigenschaften in einer [DEBUG\_PROPERTY\_INFO](../../extensibility/debugger/reference/debug-property-info.md) Struktur.  
  
## Verwalteter Code  
 Dieses Beispiel zeigt eine Implementierung von `IDebugProperty2::GetPropertyInfo` für eine Methode in verwaltetem Code lokalen's. Es zeigt auch eine Hilfsfunktion `Field.GetType`, besteht, wird der Typ des Felds abgerufen.`Field.GetValue` sehen Sie in [Auswertung der lokalen Variablen](../../extensibility/debugger/evaluating-locals.md). Die Hilfsfunktion `Field.MapModifiersToAttributes` \(nicht dargestellt\) einfach konvertiert ein Feld [FIELD\_MODIFIERS](../../extensibility/debugger/reference/field-modifiers.md) flags [DBG\_ATTRIB\_FLAGS](../../extensibility/debugger/reference/dbg-attrib-flags.md) Werte.  
...
## Nicht verwalteter Code  
 Dieses Beispiel zeigt eine Implementierung von `IDebugProperty2::GetPropertyInfo` für eine Methode in nicht verwaltetem Code lokalen's. Es zeigt außerdem zwei Hilfsfunktionen, `FieldGetType` und `FieldGetValue` werden verwendet, um des Felds Typ und Wert abzurufen. Beachten Sie, dass `VARIANT`s für den Wert des Felds verwendet werden, und geben Sie als eine `VARIANT` können eine Vielzahl von Werttypen behandeln. In dieser Implementierung `FieldGetValue` Gibt ein [IDebugField](../../extensibility/debugger/reference/idebugfield.md) Objekt, das später in einen Wert konvertiert in einem Aufruf von `FieldGetPrimitiveValue` \(wie im [Auswertung der lokalen Variablen](../../extensibility/debugger/evaluating-locals.md)\).  
