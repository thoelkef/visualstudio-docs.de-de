---
title: "Beispielimplementierung der Werte &#228;ndern | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
manager: "ghogen"
---
# Beispielimplementierung der Werte &#228;ndern
...
>  In Visual Studio 2015 ist diese Art der Implementierung von ausdrucksauswertungen veraltet. Informationen über das Implementieren von CLR\-ausdrucksauswertungen finden Sie unter [CLR\-Ausdrucksauswertungen](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) und [Managed Expression Evaluator Sample](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample).  
 ...
 Alle lokalen angezeigt, der **Lokal** verfügt über ein [IDebugProperty2](../../extensibility/debugger/reference/idebugproperty2.md) \-Objekt zugeordnet. Diese `IDebugProperty2` Objekt enthält des lokalen Namen, Wert und Typ. Wenn ein Benutzer den Wert eines lokalen ändert, ruft Visual Studio [SetValueAsString](../../extensibility/debugger/reference/idebugproperty2-setvalueasstring.md) auf den Wert der lokalen im Speicher zu aktualisieren. In diesem Beispiel wird der lokalen dargestellt, durch die `CFieldProperty` \-Klasse, die implementiert die `IDebugProperty2` Schnittstelle.  
  
> [!NOTE]
>  Für **Überwachen** und **Schnellüberwachung** Ausdrücke, wird die Wert geändert wird durch dargestellt die `CValueProperty` \-Klasse im Beispiel MyCEE. Jedoch die Implementierung von `IDebugProperty2::SetValueAsString` wie hier dargestellt ist.  
...
2.  Bindet das zugeordnete [IDebugField](../../extensibility/debugger/reference/idebugfield.md) \-Objekt an der Position im Speicher und erzeugt ein [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) Objekt.  
...
## Nicht verwalteter Code  
 Dies ist eine Implementierung von `IDebugProperty2::SetValueAsString` in verwaltetem Code. Die Hilfsfunktion `FieldCoerceValueType` \(nicht gezeigt\) erzwingt eine `VARIANT` eines bestimmten Typs und macht Sie sicher, dass der Wert ist einer der Typen `FieldSetValue` behandeln können.  
