...
Wenn Sie die `PackageRegistrationAttribute`verwenden, sollte dies automatisch erfolgen.
...
Ja. Es gibt ein nuget-Paket mit den erforderlichen Tools und Aufgaben zum Erstellen von VSIX v3-Format Manifesten in Visual Studio 2015. Fügen Sie dem Erweiterbarkeits Projekt einen Verweis auf das nuget-Paket `Microsoft.VisualStudio.Sdk.BuildTasks.14.0` hinzu. Sie müssen auch das `VsixType`-Element hinzufügen, das `v3` der Projektdatei angibt:
...
Nein, mit der VSIX V3-`InstallRoot`-Eigenschaft können Sie keine Elemente außerhalb der Visual Studio-Installationsordner Struktur installieren. Weitere Informationen finden Sie im Thema [Installieren außerhalb des Ordners "Erweiterungen](set-install-root.md) ".
