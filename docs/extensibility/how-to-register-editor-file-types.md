...
1.  Geben Sie die <xref:Microsoft.VisualStudio.Shell.ProvideEditorExtensionAttribute> Klasse mit den entsprechenden Parametern für den Editor in der Ihr VSPackage-Klasse.  
    ...
    ```  
     ...
     Die `projectGuid` ist die GUID für verschiedene Dateitypen, definiert <xref:Microsoft.VisualStudio.VSConstants.CLSID_MiscellaneousFilesProject>. Der Typ der beliebigen Datei wird bereitgestellt, so, dass die resultierende Datei nicht abgelaufen ist, Teil des Buildprozesses sein.  
