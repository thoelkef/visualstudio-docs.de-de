...
|[VSITEMID.Selection](<xref:Microsoft.VisualStudio.VSConstants.VSITEMID#Microsoft_VisualStudio_VSConstants_VSITEMID_Selection>)|Ein [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] Element-ID, die darstellt, die aktuell ausgewählte Element oder die Elemente, die den Stamm der Hierarchie enthalten kann.|
