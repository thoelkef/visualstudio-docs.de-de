...
### <a name="accessing-the-environment-font"></a>Zugreifen auf die Umgebungs Schriftart
 In nativem oder WinForms-Code kann auf die Umgebungs Schriftart durch Aufrufen der-Methode `IUIHostLocale::GetDialogFont` zugegriffen werden, nachdem die-Schnittstelle vom `SID_SUIHostLocale`-Dienst abgefragt wurde.

 Leiten Sie für Windows Presentation Foundation (WPF) die Dialogfenster Klasse von der `DialogWindow` Klasse der Shell anstelle von WPF `Window`-Klasse ab.
...
```
 ...
 (Ersetzen Sie `Microsoft.VisualStudio.Shell.11.0` durch die aktuelle Version der MPF-dll.)

 Um das Dialogfeld anzuzeigen, müssen Sie in der-Klasse über `ShowDialog()`"`ShowModal()`" aufrufen. `ShowModal()` den korrekten modalen Zustand in der Shell festlegt, stellt sicher, dass das Dialogfeld im übergeordneten Fenster zentriert ist, usw.
...
```
 ...
 `ShowModal` einen booleschen Wert zurück? (boolescher Wert, der NULL-Werte zulässt) mit dem `DialogResult`, der bei Bedarf verwendet werden kann. Der Rückgabewert ist "true", wenn das Dialogfeld mit " **OK**" geschlossen wurde.

 Wenn Sie eine WPF-Benutzeroberfläche anzeigen möchten, bei der es sich nicht um ein Dialogfeld handelt, und in einem eigenen `HwndSource`gehostet werden, z. b. ein Popup Fenster oder ein untergeordnetes WPF-Fenster eines übergeordneten Win32/WinForms-Fensters, müssen Sie die `FontFamily` und `FontSize` für das Stamm Element des WPF-Elements festlegen. (Die Shell legt die Eigenschaften im Hauptfenster fest, aber Sie werden nicht über eine `HWND`geerbt). Die Shell stellt Ressourcen bereit, an die die Eigenschaften gebunden werden können, wie folgt:
...
### <a name="BKMK_Formatting"></a>Referenz zum Formatieren (Skalieren/stärken)
...
**Prozeduraler Code:** Wenn `textBlock` ein zuvor definiertes TextBlock ist und `label` eine zuvor definierte Bezeichnung ist:
...
### <a name="testing-the-environment-font"></a>Testen der Umgebungs Schriftart
 ...
 ![Einstellungen für Schriftarten und Farben im Dialogfeld "Tools &gt; Optionen"](../../extensibility/ux-guidelines/media/0201-a_optionsfonts.png "0201-a_OptionsFonts")<br />Einstellungen für Schriftarten und Farben im Dialogfeld "Tools &gt; Optionen"
...
|Artikel, es sei denn, es handelt sich um das erste Wort im Titel|ein, ein, das|
