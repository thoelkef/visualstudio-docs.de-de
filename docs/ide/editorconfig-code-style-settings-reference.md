...
- **[Sprachkonventionen](#language)** sind Regeln für die Sprachen C# und Visual Basic, z.B. `var`/explizite Typen, Verwenden von Ausdruckskörpermembern.
- **[Formatierungsregeln](#formatting)** sind Regeln für das Layout und die Struktur Ihres Codes, um diesen einfacher lesbar zu machen, z.B. Allman-Klammern, Leerzeichen in Kontrollblöcken.
...
Für Codeformatoptionen müssen Sie **wahr** (diese Option bevorzugen) oder **falsch** (diese Option nicht bevorzugen), einen Doppelpunkt (`:`) und einen Schweregrad (`none`, `silent`, `suggestion`, `warning` oder `error`) angeben. Der Schweregrad bedeutet den Grad der Durchsetzung, den Sie für das betreffende Format in Ihrer Codebasis festlegen möchten.
...
| ------------- |:-------------:|
| **Betroffene Sprachen** | C# 7.0 und höher und Visual Basic 15 und höher
...
| ------------- |:-------------:|
| **Betroffene Sprachen** | C# 6.0 und höher
...
| ------------- |:-------------:|
| **Betroffene Sprachen** | C# 7.0 und höher
...
|  Name der Option | `csharp_prefer_simple_default_expression` |
| ------------- |:-------------:|
| **Betroffene Sprachen** | C# 7.1 und höher und Visual Basic
...
Bei Formatierungsoptionen müssen Sie **wahr** (diese Option bevorzugen) oder **falsch** (diese Option nicht bevorzugen) angeben, außer in einigen Fällen, in denen Sie stattdessen angeben müssen, unter welchen Bedingungen die Regel angewendet werden soll.
...
- **[C#-Formatierungseinstellungen](#newline)**
    ...
    - [Leerzeichenoptionen](#spacing)
        ...
        - [Leerzeichen nach Schlüsselwörtern in Anweisungen für die Ablaufsteuerung](#space_control_flow)
...
|  Name der Option | `csharp_new_line_within_query_expression_clauses` |
| ------------- |:-------------:|
| **Betroffene Sprachen** | C#
...
### <a name="space_control_flow"> Leerzeichen nach Schlüsselwörtern in Anweisungen für die Ablaufsteuerung </a>
...
Bei Benennungskonventionen müssen Sie **Symbole**, **Format** und einen **Schweregrad** angeben. Benennungskonventionen sollten von der spezifischsten zur allgemeinsten sortiert werden. Die erste Regel, die angewendet werden kann, ist die einzige Regel, die angewendet wird. 
...
Dies sind gültige Optionen für den Schweregrad einer Regel für das Benennungsformat: `none`, `silent`, `suggestion`, `warning`, `error`.
 ...
 `suggestion` bedeutet, dass dem Benutzer in der Fehlerliste und der IDE ein Vorschlag angezeigt wird. Der Schweregrad `suggetion` lässt die Ausführung der Benennungsregel zu, führt jedoch nicht zu einer Unterbrechung des Buildvorgangs.
