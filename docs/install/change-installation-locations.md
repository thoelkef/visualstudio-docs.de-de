...
2. Übernehmen Sie im Abschnitt **Visual Studio** die Standardeinstellungen. Dadurch wird das Hauptprodukt mit den Dateien installiert, die für diese Version von Visual Studio spezifisch sind.
...
4. Geben Sie im Abschnitt **Shared components, SDKs, and tools** (Freigegebene Komponenten, SDKs und Tools) das Laufwerk an, auf dem Sie Dateien speichern möchten, die von parallelen Installationen von Visual Studio freigegeben werden. SDKs und Tools, durch die der Visual Studio-Installer den Installationspfad ändern kann, werden ebenfalls in diesem Verzeichnis gespeichert.
