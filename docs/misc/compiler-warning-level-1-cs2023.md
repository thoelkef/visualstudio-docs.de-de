---
title: "Compilerwarnung (Stufe 1) CS2023 | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "CS2023"
dev_langs: 
  - "CSharp"
helpviewer_keywords: 
  - "CS2023"
ms.assetid: 2201fcbc-0eb6-4a1e-baa0-50c5968d1d92
caps.latest.revision: 9
caps.handback.revision: 9
author: "BillWagner"
ms.author: "wiwagn"
manager: "wpickett"
---
# Compilerwarnung (Stufe 1) CS2023
Die \/noconfig\-Option wird ignoriert, da sie in einer Antwortdatei angegeben wurde.  
  
 Die Compileroption [\/noconfig](/dotnet/csharp/language-reference/compiler-options/noconfig-compiler-option) wurde in einer Antwortdatei angegeben, was nicht zulässig ist.  
  
 Diese Warnung kann nicht durch die [\/nowarn](/dotnet/csharp/language-reference/compiler-options/nowarn-compiler-option)\-Option unterdrückt werden.