---
title: "Gewusst wie: definieren eine domänenspezifischen Sprache | Microsoft-Dokumentation"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
##  <a name="a-nametemplatesa-selecting-a-template-solution"></a><a name="templates"></a>Auswählen einer Vorlagenprojektmappe  
...
1.  Erstellen Sie eine Projektmappe mit der **Domain-Specific Language** Vorlage, das sich unter **andere Projekttypen/Erweiterbarkeit** in der **neues Projekt** Dialogfeld.  
...
2.  Klicken Sie auf die einzelnen Vorlagen, um eine Beschreibung anzuzeigen. Wählen Sie die Projektmappe aus, die Ihren Vorstellungen am nächsten kommt.  
    ...
    -   Wählen Sie **Aufgabenablauf** eine DSL zu erstellen, die von Verantwortlichkeitsbereichen. Verantwortlichkeitsbereiche sind vertikale oder horizontale Bereiche des Diagramms.  
...
3.  Geben Sie auf der entsprechenden Seite des Assistenten eine Dateinamenerweiterung für die DSL ein. Diese Erweiterung wird für Dateien mit Instanzen Ihrer DSL verwendet.  
    ...
    -   Wählen Sie eine Dateinamenerweiterung, die keiner Anwendung auf Ihrem Computer bzw. auf einem Computer, auf dem Sie die DSL installieren möchten, zugeordnet ist. Beispielsweise **Docx** und **Htm** wäre akzeptablen Dateinamenerweiterungen.  
...
### <a name="test-the-solution"></a>Testen der Projektmappe  
 ...
 Wenn Sie eine Lösung ausführen haben Sie von der **minimale Sprache** Vorlage, die experimentelle [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] sieht wie im folgende Beispiel:  
...
-   Die Stammklasse erscheint oben links im DSL-Definitionsdiagramm unter **Klassen und Beziehungen**. Benennen Sie sie in einen von der DSL verschiedenen Namen um. Zum Beispiel eine DSL namens **MusicLibrary** möglicherweise eine Basisklasse mit dem Namen **Musik**.  
  
-   Die Diagrammklasse erscheint unten rechts im Diagramm DSL-Definition in der **Diagrammelemente** Spalte. Möglicherweise müssen Sie einen Bildlauf nach rechts durchführen, um sie zu sehen. In der Regel heißt *Ihrdsl***Diagramm**.  
  
-   Wenn Sie verwendet die **Aufgabenfluss** und Diagramme mit Verantwortlichkeitsbereichen erstellen, halten und benennen Sie die Actor-Domänenklasse und die ActorSwimlane-Form beibehalten möchten.  
  
 Löschen Sie andere Klassen nach Bedarf, oder benennen Sie sie um.  
  
##  <a name="a-namepatternsa-patterns-for-defining-a-dsl"></a><a name="patterns"></a>Muster für das Definieren einer DSL  
...
> [!NOTE]
>  Nach dem Hinzufügen einer Funktion, vergessen Sie nicht, klicken Sie auf **alle Vorlagen transformieren** in der Symbolleiste des Projektmappen-Explorer vor dem Erstellen und Ausführen die DSL.  
...
##  <a name="a-nameclassesa-defining-domain-classes"></a><a name="classes"></a>Definieren von Domänenklassen  
 Domänenklassen stellen die Konzepte der DSL dar. Die Instanzen sind *Modellelementen*. Z. B. in einer **MusicLibrary** DSL müssen Sie möglicherweise mit dem Namen Domänenklassen **Album** und **Song**.  
  
 Zum Erstellen von einer Domänenklasse, ziehen Sie aus der **benannte Domänenklasse** -tool in das Diagramm, und benennen Sie die Klasse.  
...
|Nicht direkt angezeigt|Die Klasse, von der es Bestandteil ist||  
 ...
 Weitere Informationen finden Sie unter [Eigenschaften von Domänenbeziehungen](../modeling/properties-of-domain-relationships.md) und [Eigenschaften von Domänenrollen](../modeling/properties-of-domain-roles.md).  
...
### <a name="add-domain-properties-to-each-domain-class"></a>Hinzufügen von Domäneneigenschaften zu den einzelnen Domänenklassen  
 ...
 Klicken Sie auf **Domäneneigenschaften** in der Klasse, drücken Sie die EINGABETASTE, und geben Sie den Namen einer Eigenschaft. Der Standardtyp einer Domäneneigenschaft lautet String. Wenn Sie den Typ ändern möchten, wählen Sie die Eigenschaft "Domain" und legen Sie die **Typ** in der **Eigenschaften** Fenster. Wenn Sie den gewünschten Typ nicht in der Dropdown-Liste enthalten ist, finden Sie unter [Hinzufügen von Eigenschaftentypen](#addTypes).  
  
 **Legen Sie eine Eigenschaft als Elementnamen.** Wählen Sie eine Domäneneigenschaft, die zum Identifizieren von Elementen im Explorer Sprache verwendet werden kann. Beispielsweise könnten Sie in der "Song"-Domänenklasse die "Titel"-Domäneneigenschaft auswählen. In der **Eigenschaften** legen **ist Elementname** auf `true`.  
...
### <a name="tidy-the-dsl-definition-diagram"></a>Bereinigen des DSL-Definitionsdiagramms  
 Wenn Sie Beziehungen hinzufügen, erscheinen einige Klassen an mehreren Stellen. Reduzieren Sie die Anzahl der Darstellungen, und das Diagramm breiter zu machen, mit der rechten Maustaste der Zielklasse einer Beziehung, und klicken Sie dann auf **Struktur hier**. Den gegenteiligen Effekt, mit der rechten Maustaste der Zielklasse einer Beziehung und klicken Sie auf **Struktur teilen**. Wenn diese Menübefehle nicht angezeigt werden, vergewissern Sie sich, dass nur die Domänenklasse ausgewählt ist.  
...
1.  **Klicken Sie auf alle Vorlagen transformieren** in der Symbolleiste des Projektmappen-Explorer, um den DSL-Designercode zu generieren. Dieser Schritt kann automatisiert werden. Weitere Informationen finden Sie unter [wie alle Vorlagen transformieren automatisieren](http://msdn.microsoft.com/en-us/b63cfe20-fe5e-47cc-9506-59b29bca768a).  
...
3.  **Öffnen Sie den Explorer.** Die Seite des Diagramms befindet sich im Explorer Sprache, die in der Regel heißt *Ihresprache* Explorer. Sollten Sie das Fenster nicht sehen, befindet es sich möglicherweise auf einer Registerkarte unter dem Projektmappen-Explorer. Wenn Sie ihn finden können, auf die **Ansicht** auf **Weitere Fenster**, und klicken Sie dann auf *Ihresprache***Explorer**.  
  
     Im Explorer wird eine Strukturansicht des Modells dargestellt.  
  
4.  **Erstellen Sie neue Elemente.** Klicken Sie auf den Stammknoten oben, und klicken Sie dann auf **hinzufügen***Ihreklasse*.  
  
     Im Sprach-Explorer wird eine neue Instanz Ihrer Klasse angezeigt.  
  
5.  Überprüfen Sie beim Erstellen neuer Instanzen, ob jede Instanz einen anderen Namen hat. Dies geschieht nur, wenn Sie festgelegt haben die **ist Elementname** -Flag auf eine Domäneneigenschaft.  
  
6.  **Untersuchen Sie die Domäneneigenschaften. Mit einer Instanz der Klasse ausgewählt** das Fenster Eigenschaften zu überprüfen. Es sollte die Domäneneigenschaften zeigen, die Sie für die Domänenklasse definiert haben.  
  
7.  **Speichern Sie die Datei, schließen und öffnen Sie ihn erneut**. Nachdem Sie die Knoten erweitert haben, sollten alle erstellten Instanzen im Explorer sichtbar sein.  
  
##  <a name="a-nameshapesa-defining-shapes-on-the-diagram"></a><a name="shapes"></a>Definieren von Formen im Diagramm  
...
1.  **Definieren und Testen Sie eine Domänenklasse, wie in beschrieben**[Definieren von Domänenklassen](#classes) **.  **  
    ...
    -   Wenn das Diagramm Verantwortlichkeitsbereiche enthält, kann das übergeordnete Element die Domänenklasse sein, die einem Verantwortlichkeitsbereich zugeordnet ist. Bevor Sie mit diesem Verfahren fortfahren, finden Sie unter [Definieren einer DSL, die Verantwortlichkeitsbereichen](#swimlanes).  
  
2.  **Fügen Sie eine Shape-Klasse** um die Elemente im Modelldiagramm darzustellen. Ziehen Sie eines der folgenden Werkzeuge in das DSL-Definitionsdiagramm:  
  
    -   **Geometrie-Form** bietet ein Rechteck oder eine Ellipse.  
...
3.  **Definieren Sie ein Bild aus, wenn Sie eine Bild-Form erstellt**.  
    ...
    4.  Klicken Sie im Fenster Eigenschaften auf die **Bild** Eigenschaft.  
  
    5.  In der **Bild auswählen** Dialogfeld klicken Sie auf das Dropdown-Menü unter **Dateiname**, und wählen Sie das Bild.  
  
4.  **Fügen Sie Text-Decorator-Elemente, auf die Form, um die Domäneneigenschaften anzuzeigen.**  
  
     Sie benötigen wahrscheinlich mindestens ein Text-Decorator-Element, um den Namen oder Titel des Modellelements anzuzeigen.  
  
     Mit der rechten Maustaste in des Headers der Shape-Klasse, zeigen Sie auf **hinzufügen**, und klicken Sie dann auf **Text-Decorator**. Legen Sie den Namen des Decorator-Elements, und klicken Sie in den Eigenschaften-Fenster die **Position**.  
  
5.  **Verbinden Sie jede Form mit einer Diagrammelementzuordnung mit der Domänenklasse, die angezeigt werden sollen**.  
  
     Klicken Sie auf die **Diagrammelementzuordnung** Tools, und klicken Sie dann die Domänenklasse klicken Sie auf die Shape-Klasse.  
  
6.  **Ordnen Sie die Eigenschaften der Text-Decorator-Elemente.**  
    ...
    2.  In der **DSL-Details** Fenster, klicken Sie auf die **Decorator-Zuordnungen** Registerkarte. Wenn Sie nicht sehen die **DSL-Details** Fenster auf die **Ansicht** auf **Weitere Fenster** , und klicken Sie dann auf **DSL-Details**. Häufig muss der obere Rand des Fensters angehoben werden, um den gesamten Inhalt anzuzeigen.  
    ...
    4.  Stellen Sie sicher, dass neben jedem Decorator-Namen ein Häkchen angezeigt wird.  
     ...
     ![Formzuordnungen und DSL-Details](../modeling/media/dsldetailswindow.png "DslDetailsWindow")  
  
7.  **Stellen Sie ein Toolboxelement zum Erstellen von Elementen der Domänenklasse.**  
  
    1.  In **DSL-Explorer**, erweitern Sie die **Editor** Knoten und alle seine untergeordneten Knoten.  
  
    2.  Mit der rechten Maustaste unter des Knotens **Toolboxregisterkarten** , die den gleichen Namen wie Ihre DSL, z. B. "Musikbibliothek" hat. Klicken Sie auf **Elementwerkzeug hinzufügen**.  
  
        > [!NOTE]
        >  Wenn Sie mit der rechten Maustaste die **Tools** Knoten wird nicht angezeigt, **Elementwerkzeug hinzufügen**. Klicken Sie stattdessen auf den übergeordneten Knoten.  
  
    3.  Legen Sie im Fenster Eigenschaften mit dem neuen Elementtool ausgewählt **Klasse** mit der Domänenklasse, die vor kurzem hinzugefügt wurden.  
  
    4.  Legen Sie **Beschriftung** und **QuickInfo**.  
  
    5.  Legen Sie **Toolboxsymbol** auf ein Symbol, das in der Toolbox angezeigt werden. Sie können ein neues Symbol oder ein bereits für ein anderes Werkzeug verwendetes Symbol angeben.  
         ...
         Kehren Sie zum DSL-Definitionsdiagramm zurück, wählen Sie das Tool und klicken Sie im Fenster Eigenschaften auf **[...] ** in **Toolboxsymbol**. In der **Bitmap auswählen** wählen Sie im Dialogfeld der. BMP-Datei aus dem Dropdown-Menü.  
  
 Weitere Informationen finden Sie unter [Eigenschaften der Geometrie-Formen](../modeling/properties-of-geometry-shapes.md) und [Eigenschaften von Bildformen](../modeling/properties-of-image-shapes.md).  
  
#### <a name="to-test-shapes"></a>So testen Sie Formen  
  
1.  **Klicken Sie auf alle Vorlagen transformieren** in der Symbolleiste des Projektmappen-Explorer, um den DSL-Designercode zu generieren.  
  
2.  **Erstellen Sie und führen Sie die DSL.** Drücken Sie F5 oder STRG + F5, um eine neue Instanz der Ausführung [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] im Testmodus. Öffnen oder erstellen Sie in der experimentellen Instanz von [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] eine Datei mit der Dateinamenerweiterung Ihrer DSL.  
  
3.  **Stellen Sie sicher, dass die Elementtools in der Toolbox angezeigt werden.**  
  
4.  **Erstellen von Formen** über ein Tool in das Modelldiagramm ziehen.  
  
5.  **Stellen Sie sicher, dass jeder Text-Decorator angezeigt wird,** und:  
  
    1.  Sie bearbeiten können, es sei denn, Sie festgelegt haben die **ist schreibgeschützte Benutzeroberfläche** -Flag auf die Eigenschaft "Domain".  
...
##  <a name="a-namereferencesa-defining-reference-relationships"></a><a name="references"></a>Definieren von Verweisbeziehungen  
 ...
 **Legen Sie die Allows Duplicates-Eigenschaft der Beziehung** Wenn mehr als eine Verknüpfung der Klasse zwischen demselben Paar von Modellelementen vorhanden sein kann. Beispielsweise könnten Sie zulassen, dass ein Lehrer einen Schüler in mehreren Fächern unterrichtet.  
  
 ![Shape-Zuordnungen für Connectors](../modeling/media/music_connector.png "Music_Connector")  
...
### <a name="define-a-connector-to-display-the-relationship"></a>Definieren eines Konnektors zur Darstellung der Beziehung  
 ...
 Ziehen Sie die **Connector** Tool in der DSL-Definitionsdiagramm.  
 ...
 Öffnen Sie die diagrammelementzuordnung ausgewählt, die **DSL-Details** Fenster, und öffnen Sie die **Decorator-Zuordnungen** Registerkarte.  
  
 Wählen Sie die einzelnen **Decorator** und **Anzeigeeigenschaft** der richtigen Domäne-Eigenschaft.  
...
### <a name="define-a-connection-builder-tool"></a>Definieren eines Werkzeugs "Verbindungs-Generator"  
 In der **DSL-Explorer** Fenster, erweitern Sie die **Editor** Knoten und alle zugehörigen untergeordneten Knoten.  
  
 Mit der rechten Maustaste des Knotens mit dem gleichen Namen wie Ihre DSL, und klicken Sie dann auf **neues Verbindungstool hinzufügen**.  
...
-   Legen Sie **Toolboxsymbol** auf das Symbol, das in der Toolbox angezeigt werden soll. Sie können ein neues Symbol oder ein bereits für ein anderes Werkzeug verwendetes Symbol angeben.  
...
6.  **Stellen Sie sicher, dass Sie Verbindungen zwischen ungeeigneten Klassen erstellen können.** Beispielsweise ist die Beziehung zwischen Alben und Interpreten, überprüfen Sie, ob Sie Interpreten mit Interpreten verbinden können.  
  
7.  **Stellen Sie sicher, dass die Multiplizitäten zutreffend sind. Beispielsweise stellen Sie sicher, dass Sie eine Person mit mehr als einem Manager herstellen können.**  
  
8.  **Stellen Sie sicher, dass jeder Text-Decorator angezeigt wird,** und:  
...
##  <a name="a-namecompartmentsa-defining-shapes-that-contain-lists-compartment-shapes"></a><a name="compartments"></a>Definieren von Formen, die Listen enthalten: Depot-Formen  
 ...
 ![Formzuordnung](../modeling/media/music_mapcomp.png "Music_MapComp")  
...
1.  **Erstellen Sie die Container-Domänenklasse**. Klicken Sie auf die **einbettende Beziehung** tool, klicken Sie auf die Stammklasse des Modells, und klicken Sie dann auf einen leeren Bereich des DSL-Definitionsdiagramm. Dadurch wird in der Beispielabbildung die Domänenklasse namens "Album" erstellt.  
     ...
     Die Klasse eine Domäneneigenschaft wie Namen hinzu, und legen Sie seine **ist Elementname** Flag im Eigenschaftenfenster angezeigt.  
  
2.  **Erstellen die Liste Element Domänenklasse**. Klicken Sie auf die **einbettende Beziehung** tool, klicken Sie auf die Containerklasse (Album), und klicken Sie dann auf einen leeren Bereich des Diagramms. Dadurch wird in der Beispielabbildung die Domänenklasse namens "Song" erstellt.  
  
     Fügen Sie eine Domäneneigenschaft wie Titel, um die Klasse, und legen Sie seine **ist Elementname** Flag.  
...
3.  **Mischen Sie verschiedene Typen von Elementen in der Liste**, erstellen Sie Klassen, die von der List-Klasse erben. Die List-Klasse abstrakt machen, indem die **Vererbungsmodifizierer**.  
...
4.  **Erstellen Sie die Depot-Form**. Ziehen Sie aus der **Depot-Form** Tool in der DSL-Definitionsdiagramm.  
...
5.  Damit kann den Benutzer die listendepots Ausblenden der rechten Maustaste auf die Depot-Form, zeigen Sie auf **hinzufügen**, und klicken Sie dann auf **erweitern/reduzieren-Decorator**. Legen Sie im Eigenschaftenfenster die Position des Decorator-Elements fest.  
...
7.  Wählen Sie den Diagrammelementzuordnungs-Link zwischen der Domänenklasse und der Form aus. In der **DSL-Details** Fenster:  
    ...
    1.  Klicken Sie auf die **Decorators** Registerkarte. Klicken Sie auf den Namen des Decorator-Elements, und wählen Sie dann das entsprechende Element unter **Display-Eigenschaft**. Stellen Sie sicher, dass neben dem Namen des Decorator-Elements ein Häkchen angezeigt wird.  
  
    2.  Klicken Sie auf die **Depot-Zuordnungen** Registerkarte.  
  
         Klicken Sie auf den Namen des Depots.  
  
         Klicken Sie unter **Pfad der angezeigten Elementauflistung**, navigieren Sie zur listenelementklasse (Titel). Klicken Sie auf den Dropdownpfeil, um das Navigatorwerkzeug zu verwenden.  
...
2.  In **DSL-Explorer**, erweitern Sie die **Editor** Knoten und alle seine untergeordneten Knoten.  
  
3.  Mit der rechten Maustaste unter des Knotens **Toolboxregisterkarten** , die den gleichen Namen wie Ihre DSL, z. B. "Musikbibliothek" hat. Klicken Sie auf **Elementwerkzeug hinzufügen**.  
  
    > [!NOTE]
    >  Wenn Sie mit der rechten Maustaste die **Tools** Knoten wird nicht angezeigt, **Elementwerkzeug hinzufügen**. Klicken Sie stattdessen auf den übergeordneten Knoten.  
  
4.  Legen Sie im Fenster Eigenschaften mit dem neuen Elementtool ausgewählt **Klasse** mit der Domänenklasse, die vor kurzem hinzugefügt wurden.  
  
5.  Legen Sie **Beschriftung** und **QuickInfo**.  
  
6.  Legen Sie **Toolboxsymbol** auf ein Symbol, das in der Toolbox angezeigt werden. Sie können ein neues Symbol oder ein bereits für ein anderes Werkzeug verwendetes Symbol angeben.  
  
     Um ein neues Symbol zu erstellen, öffnen Sie dsl\ressourcen im **Projektmappen-Explorer**. Kopieren Sie eine der vorhandenen BMP-Dateien für Elementwerkzeuge, und fügen Sie sie ein. Benennen Sie die eingefügte Kopie um, und doppelklicken Sie dann darauf, um sie zu öffnen.  
  
     Kehren Sie zum DSL-Definitionsdiagramm zurück, wählen Sie das Tool und klicken Sie im Fenster Eigenschaften auf **[...] ** in **Toolboxsymbol**. In der **Bitmap auswählen** Dialogfeld Wählen Ihre BMP-Datei aus dem Dropdown-Menü.  
...
5.  Mit der rechten Maustaste in des Headers der neuen Form, und klicken Sie dann auf Add *des Listenelements.* Im Beispiel lautet der Befehl "Song hinzufügen".  
...
1.  **Hinzufügen der Depot-Form ein Depot**. Klicken Sie im Diagramm DSL-Definition Maustaste auf die Depot-Form, und zeigen Sie auf **hinzufügen**, und klicken Sie dann auf **Depot**.  
  
2.  Legen Sie **Pfad der angezeigten Elementauflistung** um zu dem Link statt zu seinem Zielelement zu navigieren. Klicken Sie auf das Dropdownmenü, und verwenden Sie die Strukturansicht, um die Verweisbeziehung statt ihres Ziels auszuwählen. Im Beispiel wird die Beziehung **"artistappearedonalbums"**.  
...
5.  **Transformieren aller Vorlagen**, erstellen und führen Sie die DSL, und öffnen Sie ein Testmodell.  
...
##  <a name="a-nameportsa-defining-ports-on-the-boundary-of-another-shape"></a><a name="ports"></a>Definieren von Anschlüssen am Rand einer anderen Form  
 ...
 Ein Beispiel finden Sie die Ports verwendet, wählen Sie die **Komponentendiagramm** Vorlage, wenn Sie eine neue DSL-Projektmappe erstellen. Dieses Beispiel zeigt die wesentlichen Punkte, die Sie bei der Definition von Anschlüssen berücksichtigen können:  
...
-   Die Anschluss-Domänenklassen werden Anschluss-Formen zugeordnet. Sie können entweder die abgeleiteten Klassen verschiedenen Anschluss-Formklassen zuordnen oder die Basisklasse einer Anschluss-Formklasse zuordnen.  
 ...
 In anderer Hinsicht Anschlussformen Verhalten sich wie beschrieben [Definieren von Formen](#shapes).  
  
 Weitere Informationen finden Sie unter [Eigenschaften von Anschlussformen](../modeling/properties-of-port-shapes.md).  
  
##  <a name="a-nameswimlanesa-defining-a-dsl-that-has-swimlanes"></a><a name="swimlanes"></a>Definieren einer DSL, die von Verantwortlichkeitsbereichen  
...
##  <a name="a-nameaddtypesa-adding-property-types"></a><a name="addTypes"></a>Hinzufügen von Eigenschaftentypen  
...
### <a name="domain-enumerations-and-literals"></a>Domänenenumerationen und Literale  
 ...
 Um die domänenenumeration enumerationsliterale hinzuzufügen, Maustaste die domänenenumeration in der **DSL-Explorer** , und klicken Sie dann auf **neues Enumerationsliteral hinzufügen**.  
  
 Eine Eigenschaft, die einen Enumerationstyp besitzt, kann standardmäßig jeweils nur auf einen Wert der Enumeration festgelegt werden. Wenn Sie Benutzer und Programmierer eine beliebige Kombination von Werten - festlegen möchten als "Bitfeld" - festgelegt der **IsFlags** Eigenschaft der Enumeration.  
  
### <a name="external-types"></a>Externe Typen  
 Wenn Sie den Typ einer Domäneneigenschaft festlegen, wenn der Typ nicht gefunden wird, Sie möchten in der **Typ** Dropdown Liste können Sie einen externen Typ hinzufügen. Sie könnten z. B. Hinzufügen der **System.Drawing.Color** zur Liste.  
  
 Um einen Typ hinzuzufügen, mit der rechten Maustaste in den Stamm des Modells im DSL-Explorer, und klicken Sie dann auf **neuen externen Typ hinzufügen**. Legen Sie im Fenster Eigenschaften den Namen zu **Farbe** und den Namespace auf **System.Drawing**. Dieser Typ wird jetzt im DSL-Explorer unter **Domänentypen**. Sie können ihn immer auswählen, wenn Sie den Typ einer Domäneneigenschaft festlegen.  
  
##  <a name="a-namecustoma-customizing-the-dsl"></a><a name="custom"></a>Anpassen der DSL  
...
### <a name="changing-the-dsl-definition"></a>Ändern der DSL-Definition  
 ...
 Siehe auch [Gewusst wie: Ändern Sie den Namespace einer domänenspezifischen Sprache](../modeling/how-to-change-the-namespace-of-a-domain-specific-language.md).  
  
##  <a name="a-nametroublea-troubleshooting"></a><a name="trouble"></a>Problembehandlung  
 In der folgenden Tabelle sind einige der häufigsten Probleme, die beim Entwurf einer DSL auftreten, zusammen mit ihrer Lösung aufgeführt. Weitere Hinweise finden Sie auf der [Forum zur Erweiterbarkeit von Visualisierung Tools](http://go.microsoft.com/fwlink/?LinkId=186074).  
  
|Problem|Vorschlag|  
|-------------|----------------|  
|Die Änderungen, die ich an der DSL-Definition vorgenommen habe, wirken sich nicht aus.|Klicken Sie auf **alle Vorlagen transformieren** auf der Symbolleiste über den Projektmappen-Explorer und dann neu erstellen die Projektmappe.|  
|Formen zeigen statt des Eigenschaftenwerts den Namen eines Decorator-Elements an.|Richten Sie die Decorator-Zuordnung ein. Klicken Sie im DSL-Definitionsdiagramm auf die Diagrammelementzuordnung (die graue Linie zwischen der Domänenklasse und der Formklasse).<br /><br /> Öffnen Sie die **DSL-Details** Fenster. Wenn Sie es im Menü Ansicht nicht angezeigt wird, zeigen Sie auf **Weitere Fenster**, und klicken Sie dann auf **DSL-Details**.<br /><br /> Klicken Sie auf die **Decorator-Zuordnungen** Registerkarte. Wählen Sie den Namen des Decorator-Elements aus. Vergewissern Sie sich, dass das Kontrollkästchen daneben aktiviert ist. Klicken Sie unter **Anzeigeeigenschaft**, wählen Sie den Namen einer Domäneneigenschaft.<br /><br /> Weitere Informationen finden Sie unter [Formen im Diagramm](#shapes).|  
...
|Im Explorer für meine DSL werden die Elemente nur mit ihren Typennamen angezeigt.|Wählen Sie in der DSL-Definition eine Domäneneigenschaft der Klasse, und legen Sie im Fenster **ist Elementname** auf "true".|  
|Meine DSL wird immer im XML-Editor geöffnet.|Das kann an einem Fehler beim Lesen der Datei liegen. Nachdem Sie den Fehler behoben haben, müssen Sie den Editor explizit als Ihren DSL-Designer zurücksetzen.<br /><br /> Mit der rechten Maustaste des Projektelements, klicken Sie auf **Öffnen mit** , und wählen Sie *Ihresprache***Designer (Standard)**.|  
|Der Werkzeugkasten meiner DSL wird nicht angezeigt, nachdem ich die Assemblynamen geändert habe.|Untersuchen und aktualisieren **DslPackage\GeneratedCode\Package.tt** Weitere Informationen finden Sie unter [Gewusst wie: Ändern Sie den Namespace einer domänenspezifischen Sprache](../modeling/how-to-change-the-namespace-of-a-domain-specific-language.md).|  
|Der Werkzeugkasten meiner DSL wird nicht angezeigt, obwohl ich die Assemblynamen nicht geändert habe.<br /><br /> Oder es wird in einem Meldungsfeld gemeldet, dass eine Erweiterung nicht geladen werden konnte.|Setzen Sie die experimentelle Instanz zurück, und erstellen Sie die Projektmappe neu.<br /><br /> 1.  Im Windows-Startmenü unter **Programme**, erweitern Sie [!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)], dann **Tools**, und klicken Sie dann auf **Zurücksetzen der Microsoft Visual Studio experimentelle Instanz**.<br />2.  Auf der [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] **erstellen** Menü klicken Sie auf **Projektmappe neu erstellen**.|  
