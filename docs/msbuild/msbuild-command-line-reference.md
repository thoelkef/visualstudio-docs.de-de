...
|/binaryLogger[:[LogFile=]`output.binlog`[;ProjectImports=[None,Embed,ZipFile]]]|/bl|Serialisiert alle Buildereignisse in eine komprimierte Binärdatei. Standardmäßig befindet sich die Datei im aktuellen Verzeichnis und heißt `msbuild.binlog`. Das binäre Protokoll ist eine ausführliche Beschreibung des Buildprozesses, der später für die Wiederherstellung von Textprotokollen und von weiteren Analysetools verwendet werden kann. Ein binäres Protokoll ist in der Regel zehn- bis zwanzigmal kleiner als die ausführlichste Textprotokoll auf Diagnoseebene. Es enthält aber mehr Informationen.<br /><br />Die binäre Protokollierung erfasst standardmäßig den Quelltext von Projektdateien, einschließlich aller importierten Projekte und Zieldateien, die während des Builds auftreten. Der optionale Schalter `ProjectImports` steuert dieses Verhalten:<br /><br /> -   **ProjectImports=None**. Erfassen Sie die Projektimporte nicht.<br /> -   **ProjectImports=Embed**. Betten Sie Projektimporte in die Protokolldatei (Standard) ein.<br /> -   **ProjectImports=ZipFile**. Speichern Sie Projektdateien auf `output.projectimports.zip`, wo `output` den gleichen Namen wie die binäre Protokolldatei hat.<br /><br />Die Standardeinstellung für „ProjectImports“ ist „Einbetten“.<br />**Hinweis:** Die Protokollierung erfasst keine Nicht-MSBuild-Quelldateien wie `.cs`, `.cpp` usw.<br />Eine `.binlog`-Datei kann „erneut wiedergegeben“ werden, indem statt einem Projekt/einer Projektmappe als Argument an `msbuild.exe` übergeben wird. Andere Protokollierungen erhalten die in der Protokolldatei enthaltenen Informationen, als ob der ursprüngliche Build erstellt würde. Weitere Informationen über die binäre Protokollierung und ihre Verwendungen finden Sie unter: https://github.com/Microsoft/msbuild/wiki/Binary-Log. <br /><br />**Beispiele**:<br /> -   `/bl`<br /> -    `/bl:output.binlog`<br /> -   `/bl:output.binlog;ProjectImports=None`<br /> -   `/bl:output.binlog;ProjectImports=ZipFile`<br /> -   `/bl:..\..\custom.binlog`<br /> -   `/binaryLogger`|
