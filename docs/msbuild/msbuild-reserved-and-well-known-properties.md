...
| `MSBuildVersion` | Reserviert | Die Version von [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)], das zum Erstellen des Projekts verwendet wird. <br /> <br/>Diese Eigenschaft kann nicht überschrieben werden, ansonsten wird die Fehlermeldung `MSB4004 - The "MSBuildVersion" property is reserved, and cannot be modified.` zurückgegeben. |
