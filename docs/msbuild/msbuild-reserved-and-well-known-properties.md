---
title: "Reservierte und bekannte Eigenschaften f&#252;r MSBuild | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "MSBuild reservierte Eigenschaften"
...
|--------------|-----------------|-----------------------------|  
|`MSBuildBinPath`|Der absolute Pfad des Ordners, in dem die [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)] derzeit verwendeten-Binärdateien befinden (z. B. C:\Windows\Microsoft.Net\Framework\\*VersionNumber*). Diese Eigenschaft ist nützlich, wenn Sie auf Dateien im [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)]-Verzeichnis verweisen müssen.<br /><br /> Schließen Sie nicht den abschließenden umgekehrten Schrägstrich in dieser Eigenschaft ein.|Reserviert|  
...
`MSBuildExtensionsPath64`|Der Pfad der [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)] Unterordner unter dem Ordner \Programme. Bei einem 64-Bit-Computer zeigt dieser Pfad immer auf den Ordner "\Programme". Bei einem 32-Bit-Computer ist dieser Pfad leer. Siehe auch `MSBuildExtensionsPath` und `MSBuildExtensionsPath32`.<br /><br /> Schließen Sie nicht den abschließenden umgekehrten Schrägstrich in dieser Eigenschaft ein.|Bekannt|  
|`MSBuildLastTaskResult`|`true`, wenn die vorherige Aufgabe ohne Fehler (auch wenn Warnungen auftraten) abgeschlossen wurde, oder `false`, wenn bei der vorherigen Aufgabe Fehler aufgetreten sind. Wenn ein Fehler in einer Aufgabe auftritt, ist das Auftreten des Fehlers in der Regel das letzte Ereignis in dem jeweiligen Projekt. Daher ist der Wert dieser Eigenschaft niemals `false`, außer in diesen Szenarien:<br /><br /> – Wenn der `ContinueOnError` -Attribut des der [Task-Element (MSBuild)](../msbuild/task-element-msbuild.md) Wert `WarnAndContinue` (oder `true`) oder `ErrorAndContinue`.<br /><br /> – Wenn der `Target` verfügt über eine [OnError-Element (MSBuild)](../msbuild/onerror-element-msbuild.md) als untergeordnetes Element.|Reserviert|  
|`MSBuildNodeCount`|Die maximale Anzahl nebenläufiger Prozesse, die beim Erstellen verwendet werden. Dies ist der Wert, der für angegebene **maxcpucount** in der Befehlszeile. Wenn Sie angegeben **maxcpucount** ohne einen Wert anzugeben, klicken Sie dann `MSBuildNodeCount` Gibt die Anzahl der Prozessoren im Computer an. Weitere Informationen finden Sie unter [-Befehlszeilenreferenz](../msbuild/msbuild-command-line-reference.md) und [Erstellen von mehreren Projekten gleichzeitig](../msbuild/building-multiple-projects-in-parallel-with-msbuild.md).|Reserviert|  
...
|`MSBuildThisFileFullPath`|Der absolute Pfad des Projekts oder der TARGETS-Datei, die das gerade ausgeführte Ziel enthält.<br /><br /> Tipp: Sie können einen relativen Pfad in einer Targets-Datei angeben, die relativ zur Targets-Datei und nicht relativ zur ursprünglichen Projektdatei ist.|Reserviert|  
...
|`MSBuildToolsVersion`|Die Version des [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)]-Toolsets, das zum Erstellen des Projekts verwendet wird.<br /><br /> Hinweis: Ein [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)] Toolset besteht aus Aufgaben, Zielen und Tools, die zum Erstellen einer Anwendung verwendet werden. Zu den Tools zählen Compiler wie "csc.exe" und "vbc.exe". Weitere Informationen finden Sie unter [Toolset (ToolsVersion)](../msbuild/msbuild-toolset-toolsversion.md), und [Standard- und benutzerdefinierte Toolsetkonfigurationen](../msbuild/standard-and-custom-toolset-configurations.md).|Reserviert|  
