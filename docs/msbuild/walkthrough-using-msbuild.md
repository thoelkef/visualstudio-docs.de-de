---
title: "Exemplarische Vorgehensweise: Verwenden von MSBuild | Microsoft Docs"
ms.custom: ""
ms.date: "12/02/2016"
...
3.  In der **Neues Projekt** Wählen Sie im Dialogfeld der Visual C#-Projekttyp, und wählen Sie dann die **Windows Forms-Anwendung** Vorlage. In den **Namen** Geben `BuildApp`. Geben Sie einen **Speicherort** für die Projektmappe ein, z. B. `D:\`. Übernehmen Sie die Standardeinstellungen für **Projektmappenverzeichnis erstellen** (ausgewählt), **zur Versionskontrolle hinzufügen** (nicht ausgewählt), und **Projektmappenname** (`BuildApp`).  
  
     Klicken Sie auf **OK** der Projektdatei erstellen.  
  
## <a name="examining-the-project-file"></a>Untersuchen der Projektdatei  
 Im vorherigen Abschnitt haben Sie in Visual Studio eine Visual C#-Projektdatei erstellt. Die Projektdatei wird im dargestellt **Projektmappen-Explorer** durch den Projektknoten BuildApp. Sie können die Projektdatei im Visual Studio Code-Editor untersuchen.  
  
#### <a name="to-examine-the-project-file"></a>So überprüfen Sie die Projektdatei  
  
1.  In **Projektmappen-Explorer**, klicken Sie auf den Projektknoten BuildApp.  
  
2.  In der **Eigenschaften** Browser, beachten Sie, dass die **Projektdatei** Eigenschaft BuildApp.csproj angezeigt wird. Alle Projektdateien werden mit dem Suffix "proj" benannt. Wenn Sie ein Visual Basic-Projekt erstellt hätten, wäre der Projektdateiname BuildApp.vbproj.  
  
3.  Maustaste auf den Projektknoten, und klicken Sie auf **Projekt entladen**.  
  
4.  Mit der rechten Maustaste erneut auf des Projektknotens, und klicken Sie dann **BuildApp.csproj bearbeiten**.  
  
     Die Projektdatei wird im Code-Editor angezeigt.  
  
## <a name="targets-and-tasks"></a>Ziele und Aufgaben  
 Projektdateien sind Dateien im XML-Format mit dem Stammknoten [Projekt](../msbuild/project-element-msbuild.md).  
...
```  
 ...
 Das Erstellen einer Anwendung erfolgt mit [Ziel](../msbuild/target-element-msbuild.md) und [Aufgabe](../msbuild/task-element-msbuild.md) Elemente.  
  
-   Eine Aufgabe bildet die kleinste Arbeitseinheit, d. h. das „Atom“ eines Builds. Aufgaben sind unabhängige ausführbare Komponenten, die über Eingaben und Ausgaben verfügen können. Derzeit wird in der Projektdatei nicht auf Aufgaben verwiesen, und solche wurden nicht definiert. In den folgenden Abschnitten fügen Sie der Projektdatei Aufgaben hinzu. Weitere Informationen finden Sie unter der [Aufgaben](../msbuild/msbuild-tasks.md) Thema.  
...
-   Als Ziel wird eine benannte Sequenz von Aufgaben bezeichnet. Am Ende der Projektdatei befinden sich zwei Ziele, die derzeit in HTML-Kommentare eingeschlossen sind: BeforeBuild und AfterBuild.  
    ...
    ```  
     ...
     Weitere Informationen finden Sie unter der [Ziele](../msbuild/msbuild-targets.md) Thema.  
...
```  
 ...
 Das Ziel Build ist nicht in der Projektdatei definiert. Es wird stattdessen aus der Datei Microsoft.CSharp.targets importiert, mit der [Import](../msbuild/import-element-msbuild.md) Element.  
...
## <a name="building-the-target"></a>Erstellen des Ziels  
 Führen Sie MSBuild an der **Visual Studio Command Prompt** das oben definierte Ziel HelloWorld zu erstellen. Verwenden Sie den Befehlszeilenschalter /target oder /t, um das Ziel auszuwählen.  
  
> [!NOTE]
>  Wir bezeichnen die **Visual Studio Command Prompt** als die **Befehlsfenster** in den folgenden Abschnitten.  
  
#### <a name="to-build-the-target"></a>So erstellen Sie das Ziel  
  
1.  Klicken Sie auf **Start**, klicken Sie dann auf **Programme**. Suchen und klicken Sie auf die **Visual Studio Command Prompt** in die **Visual Studio-Tools** Ordner.  
...
4.  Überprüfen Sie die Ausgabe in die **Befehlsfenster**. Die beiden Zeilen "Hello" und "World" sollten angezeigt werden:  
...
> [!NOTE]
>  Wenn Sie stattdessen feststellen `The target "HelloWorld" does not exist in the project` haben Sie wahrscheinlich vergessen, die Projektdatei im Code-Editor zu speichern. Speichern Sie die Datei, und versuchen Sie es erneut.  
...
3.  Aus der **Befehlsfenster**, geben Sie ein, und führen Sie die folgende Zeile:  
...
```  
 ...
 Fast alle MSBuild-Elemente können ein Condition-Attribut besitzen. Nähere Informationen zur Verwendung der Condition-Attribut finden Sie unter [Bedingungen](../msbuild/msbuild-conditions.md).  
  
### <a name="reserved-properties"></a>Reservierte Eigenschaften  
 Einige Eigenschaftennamen werden von MSBuild reserviert, um Informationen zur Projektdatei und zu den Binärdateien von MSBuild zu speichern. Ein Beispiel für eine reservierte Eigenschaft ist "MSBuildToolsPath". Auf reservierte Eigenschaften wird wie auf jede andere Eigenschaft mit der $-Notation verwiesen. Weitere Informationen finden Sie unter [Gewusst wie: Verweisen auf den Namen oder Speicherort der Projektdatei](../msbuild/how-to-reference-the-name-or-location-of-the-project-file.md) und [MSBuild reservierte und bekannte Eigenschaften](../msbuild/msbuild-reserved-and-well-known-properties.md).  
...
1.  Aus der **Befehlsfenster**, geben Sie ein, und führen Sie die folgende Zeile:  
...
### <a name="include-exclude-and-wildcards"></a>Include, Exclude und Platzhalter  
 Sie können das Platzhalterzeichen "*","\*\*", und "?" mit dem Include-Attribut, um einem Elementtyp Elemente hinzuzufügen. Beispiel:  
...
```  
 ...
 alle Dateien mit der Dateierweiterung ".jpeg" im Bilderordner und allen Unterordnern dem Photos-Elementtyp hinzufügt. Weitere Beispiele finden Sie unter [Gewusst wie: Auswählen von Dateien für den Buildvorgang](../msbuild/how-to-select-the-files-to-build.md).  
...
4.  Aus der **Befehlsfenster**, geben Sie ein, und führen Sie die folgende Zeile:  
...
```  
 ...
 Beispielsweise kann eine Elementliste von Quelldateien über einen Ausdruck, z. B. `@(SourceFiles -> '%(Filename).obj')`, in eine Auflistung von Objektdateien transformiert werden. Weitere Informationen finden Sie unter [transformiert](../msbuild/msbuild-transforms.md).  
...
## <a name="whats-next"></a>Weitere Informationen  
 Probieren Sie aus, um weitere Informationen zum Erstellen ein einfaches Projekt schrittweisen der [Exemplarische Vorgehensweise: Erstellen einer MSBuild-Projektdatei von Grund auf Neu](../msbuild/walkthrough-creating-an-msbuild-project-file-from-scratch.md).  
  
## <a name="see-also"></a>Siehe auch
[Übersicht über MSBuild](../msbuild/msbuild1.md)  
