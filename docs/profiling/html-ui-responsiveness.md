---
title: "HTML-UI-Reaktionsf&#228;higkeit | Microsoft Docs"
ms.custom: ""
ms.date: "12/02/2016"
...
dev_langs: 
  ...
  - "C++"
helpviewer_keywords: 
  - "Leistung, JavaScript [Windows Store-Apps]"
  - "Leistungstools, JavaScript [Windows Store-Apps]"
  - "UI-Reaktionsfähigkeit-Profiler [JavaScript]"
  - "Profiler, Reaktionsfähigkeit der Benutzeroberfläche [JavaScript]"
...
4.  Wenn Sie den Benutzeroberflächen\-Reaktionsfähigkeits\-Profiler starten, werden Sie im Fenster „Benutzerkontensteuerungsfenster“ eventuell aufgefordert, die Berechtigung zur Ausführung von Visual Studio ETW Collector.exe anzugeben. Klicken Sie auf **Ja**.  
     ...
     Interagieren mit der App, um das relevante Leistungsszenario zu testen. Einen ausführlichen Workflow finden Sie unter [Isolieren eines Problems mit der Reaktionsfähigkeit der Benutzeroberfläche](#Workflow) und [Isolieren eines Problems mit dem visuellen Durchsatz](#IsolateVisualThroughput).  
...
4.  Optional können Sie dem Code mit [Markieren von Code zur Analyse](#ProfileMark) Benutzermarkierungen hinzufügen.  
...
8.  Wenn Sie Benutzermarkierungen hinzugefügt haben, werden diese in der [Anzeigen einer Zeitachse für die Diagnosesitzung](#Ruler) des Profilers angezeigt. Die folgende Abbildung zeigt eine einzelne Benutzermarkierung, die verwendet wird, um einen bestimmten Vorgang im Code anzugeben.  
     ...
     ![Diagnoselineal mit einer Benutzermarkierung](../profiling/media/js_htmlvizprofiler_usermark.png "JS\_HTMLVizProfiler\_UserMark")  
...
10. Wählen Sie in einem der Diagramme den relevanten Bereich aus, indem Sie auf einen Teil des Diagramms klicken und den Mauszeiger ziehen \(oder mit der TABTASTE und den PFEILTASTEN\). Wenn Sie einen Zeitraum auswählen, indem Sie eine Auswahl treffen, ändert sich das Zeitachsendetaildiagramm im unteren Bereich des Profilers, um nur den ausgewählten Zeitraum anzuzeigen.  
     ...
     ![CPU&#45;Auslastungsdiagramm](../profiling/media/js_htmlvizprof_cpu_util.png "JS\_HTMLVizProf\_CPU\_Util")  
  
11. Verwenden Sie das [Anzeigen des Zeitachsendetaildiagramms](#TimelineDetails), um ausführliche Informationen über Ereignisse abzurufen, die entweder zu häufig ausgeführt werden oder zu viel Zeit in Anspruch nehmen. Suchen Sie z. B. nach Folgendem:  
    ...
    -   Andere in der [Profilerereignisreferenz](#ProfilerEvents) angegebene Ereignisse.  
...
```  
 ...
 ![Benutzer&#45;Messgrößenereignis in der Zeitleistendetailansicht](../profiling/media/js_htmlvizprofiler_user_measure.png "JS\_HTMLVizProfiler\_User\_Measure")  
...
###  <a name="Ruler"></a> Anzeigen einer Zeitachse für die Diagnosesitzung  
 ...
 ![Lineal der Diagnosesitzung](../profiling/media/js_htmlvizprof_ruler.png "JS\_HTMLVizProf\_Ruler")  
...
###  <a name="CPUUtilization"></a> Anzeigen der CPU\-Auslastung  
 Das Diagramm der CPU\-Auslastung ermöglicht das Identifizieren von Zeiträumen mit übermäßiger CPU\-Aktivität. Es stellt Informationen über die durchschnittliche CPU\-Auslastung der App über einen bestimmten Zeitraum dar. Die Informationen sind farbcodiert, um die folgenden spezifischen Kategorien darzustellen: **Laden**, **Skripterstellung**, Garbage Collection \(**GC**\), **Format**, **Rendern** und **Bilddekodierung**. Weitere Informationen über diese Kategorien finden Sie unter [Profilerereignisreferenz](#ProfilerEvents) in diesem Thema.  
...
-   Sie erhalten eine ausführlichere Ansicht eines ausgewählten Zeitraums, indem Sie die Schaltfläche **Vergrößern** auswählen.  
 ...
 Weitere Informationen zur Verwendung des Diagramms finden Sie unter [Isolieren eines Problems mit der Reaktionsfähigkeit der Benutzeroberfläche](#Workflow) in diesem Thema.  
...
-   Das Diagramm zeigt den Wert Null an, wenn Frames abgelegt werden.  
 ...
 ![Diagramm des visuellen Durchsatzes](../profiling/media/js_htmlvizprof_vizthru.png "JS\_HTMLVizProf\_VizThru")  
...
>  Wenn ein Ereignis in einem Hintergrundthread auftritt, wird die Thread\-ID in Klammern neben dem Ereignisnamen angezeigt.  
 ...
 ![Detaildiagramm der Zeitachse](../profiling/media/js_htmlvizprof_timelinedet.png "JS\_HTMLVizProf\_TimelineDet")  
 ...
 Die Ereignisse im Zeitachsendetaildiagramm sind farbcodiert und stellen dieselben Kategorien von Aufgaben dar, die im CPU\-Auslastungsdiagramm angezeigt werden. Weitere Informationen über die Ereigniskategorien und die spezifischen Ereignisse finden Sie unter [Profilerereignisreferenz](#ProfilerEvents) in diesem Thema.  
...
###  <a name="FilterTimelineDetails"></a> Filtern der Zeitachsendetails  
 ...
 ![Filtern der Zeitachse nach einem Ereignis](../profiling/media/js_htmlvizprofiler_filtertoevent.png "JS\_HTMLVizProfiler\_FilterToEvent")  
...
###  <a name="FilterEvents"></a> Filtern von Ereignissen  
 ...
 ![Filtern von Ereignissen in der Zeitleiste](../profiling/media/js_htmlvizprofiler_event_filter.png "JS\_HTMLVizProfiler\_Event\_Filter")  
...
###  <a name="GroupFrames"></a> Ereignisse nach Frame gruppieren  
 ...
 ![Ereignisse auf der obersten Ebene gruppieren](../profiling/media/js_htmlvizprofiler_frame_grouping_button.png "JS\_HTMLVizProfiler\_Frame\_Grouping\_Button")  
 ...
 ![Zeitachsenereignisse gruppiert nach Rahmen](../profiling/media/js_htmlvizprofiler_frame_grouping.png "JS\_HTMLVizProfiler\_Frame\_Grouping")  
...
|--------------|-----------------------|-------------------|  
|CSS\-Analyse|Ladevorgang|Neuer CSS\-Inhalt wurde gefunden, und es wurde versucht, den CSS\-Inhalt zu analysieren.|  
...
|HTTP\-Anforderung|Ladevorgang|Eine Remoteressource wurde im DOM gefunden, oder ein XMLHttpRequest wurde erstellt, das zu einer HTTP\-Anforderung geführt hat.|  
|Spekulatives Herunterladen|Ladevorgang|Der HTML\-Inhalt der Seite wurde für erforderliche Ressourcen gefunden, sodass darauf folgende HTTP\-Anforderungen für die Ressourcen schnell geplant werden konnten.|  
...
|CSS\-Berechnung|Formatieren|Es wurden Änderungen am DOM vorgenommen, für die die Formateigenschaften aller betroffenen Elemente neu berechnet werden musste.|  
|Layout|Formatieren|Es wurden Änderungen am DOM vorgenommen, für die die Größe und\/oder die Position aller betroffenen Elemente neu berechnet werden musste.|  
...
|Decodieren von Bildern|Decodieren von Bildern|Ein Bild wurde im DOM eingefügt, und es wurde versucht, das Bild vom ursprünglichen Format in eine Bitmap zu dekomprimieren und zu decodieren.|  
