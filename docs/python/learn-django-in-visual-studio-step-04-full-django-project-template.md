---
title: 'Tutorial: Informationen zu Django in Visual Studio – Schritt 4'
...
1. Wechseln Sie in Visual Studio zum **Projektmappen-Explorer**. Klicken Sie mit der rechten Maustaste auf die Projektmappe „LearningDjango“, die Sie in diesem Tutorial bereits zuvor erstellt haben, und klicken Sie auf **Hinzufügen** > **Neues Projekt**. (Wenn Sie stattdessen eine neue Projektmappe verwenden möchten, klicken Sie auf **Datei** > **Neu** > **Projekt**.)
...
1. Wählen Sie zum Ausführen des Servers **Debuggen** > **Debuggen starten** (F5) aus, oder verwenden Sie hierzu die Schaltfläche **Webserver** auf der Symbolleiste:
    ...
    ![Schaltfläche zum Ausführen des Webservers auf der Symbolleiste in Visual Studio](media/django/run-web-server-toolbar-button.png)
...
Die einzelnen Seitenvorlagen `about.html`, `contact.html` und `index.html` erweitern jeweils die Basisvorlage `layout.html`. `about.html` ist die einfachste Vorlage. Sie enthält die Tags `{% extends %}` und `{% block content %}`:
...
`index.html` und `contact.html` verwenden die gleiche Struktur. Ihr Inhalt im Block „content“ ist umfangreicher.

Der Ordner `templates/app` beinhaltet auch eine vierte Seite `login.html`. Zudem enthält er `loginpartial.html`, das mithilfe von `{% include %}` in `layout.html` eingebunden wird. Diese Vorlagendateien werden bei der Authentifizierung in Schritt 5 näher erläutert.
...
Die `urls.py`-Datei des Django-Projekts, die von der Vorlage „Django-Webprojekt“ erstellt wurde, enthält den folgenden Code:
...
Die ersten drei URL-Muster werden direkt den Ansichten `home`, `contact` und `about` in der `views.py`-Datei der App zugeordnet. Die Muster `^login/$` und `^logout$` verwenden hingegen integrierte Django-Ansichten anstelle der von der App definierten Ansichten. Auch die Aufrufe der `url`-Methode enthalten zusätzliche Daten, um die Ansicht anzupassen. Diese Aufrufe werden in Schritt 5 näher beschrieben.
...
- Quellcode des Tutorials auf GitHub unter [Microsoft/python-sample-vs-learn-django](https://github.com/Microsoft/python-sample-vs-learn-django)