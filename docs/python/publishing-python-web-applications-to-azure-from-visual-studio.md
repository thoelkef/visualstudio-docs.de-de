...
ms.locfileid: "39468669"
---
# <a name="publish-to-azure-app-service"></a>Veröffentlichen in Azure App Service

> [!Important]
> Die Bereitstellung von Python-Apps für Azure App Service für Linux wird derzeit von Visual Studio nicht unterstützt. Außerdem plant Microsoft, Python für App Service unter Windows nicht mehr zu unterstützen. Wenn es dazu Neuigkeiten gibt, werden Sie hier darüber informiert. In der Zwischenzeit können Sie Bereitstellungen für App Service unter Linux mithilfe von Containern ausführen. Weitere Informationen finden Sie unter [Bereitstellen einer Python-Web-App über die Web-App für Container](/azure/app-service/containers/quickstart-python).

Visual Studio bietet Ihnen die Möglichkeit, Python-Web-Apps direkt in Azure App Service zu veröffentlichen. Wenn Sie eine App in Azure App Service veröffentlichen möchten, müssen Sie die benötigten Dateien auf den Server kopieren und eine *web.config*-Datei einrichten, die Anweisungen für den Webserver zum Starten der App enthält.
...
1. Wählen Sie in Visual Studio **Datei** > **Neu** > **Projekt** aus, suchen Sie nach „Bottle“, wählen Sie das **Bottle Web Project** aus, geben Sie einen Namen und einen Pfad für das Projekt an, und klicken Sie auf **OK**. (Die Bottle-Vorlage ist Bestandteil der Python-Entwicklungsworkload. Informationen dazu finden Sie unter [Installation](installing-python-support-in-visual-studio.md).)
...
1. Testen Sie das Projekt lokal, indem Sie **F5** drücken oder **Debuggen** > **Debuggen starten** auswählen.
...
### <a name="use-a-subscription"></a>Verwenden eines Abonnements
...
### <a name="use-a-temporary-app-service"></a>Verwenden eines temporären App Service
...
1. Wählen Sie in der angezeigten Registerkarte **Veröffentlichen** das Ziel für die Veröffentlichung aus:
    ...
    1. Ihr eigenes Azure-Abonnement: Klicken Sie auf **Microsoft Azure App Service** und dann auf **Vorhandenen auswählen** > **Veröffentlichen**. Ein Dialogfeld wird angezeigt, in dem Sie das passende Abonnement und den passenden App-Dienst auswählen. Wenn App Service nicht angezeigt wird, verwenden Sie, wie unten beschrieben, das heruntergeladene Veröffentlichungsprofil für einen temporären App-Dienst.

       ![Veröffentlichen in Azure, Schritt 1, Visual Studio 2017: vorhandene Abonnements](media/tutorials-common-publish-1a-2017.png)

    2. Wenn Sie auf „try.azurewebsites.net“ ein temporäres App Service verwenden oder stattdessen ein anderes Veröffentlichungsprofil verwenden müssen, klicken Sie auf das Steuerelement **\>**, um nach **Profil importieren** zu suchen. Wählen Sie diese Option aus, und klicken Sie dann auf **Veröffentlichen**. Dann wird die Eingabe des Speicherorts der zuvor heruntergeladenen *.publishsettings*-Datei verlangt.
...
1. Wenn der Browser geöffnet wird, sollte die Meldung **Die Seite kann aufgrund eines internen Serverfehlers nicht angezeigt werden.** angezeigt werden. Diese Meldung deutet darauf hin, dass die Python-Umgebung auf dem Server nicht vollständig konfiguriert wurde. Führen Sie in diesem Fall folgende Schritte aus:
    ...
    1. Lesen Sie erneut den Artikel [Verwalten von Python auf Azure App Service](managing-python-on-azure-app-service.md), um sicherzustellen, dass eine passende Websiteerweiterung für Python installiert ist.
    ...
    5. Versuchen Sie, App Service nach der Installation neuer Pakete neu zu starten. Wenn Sie die *web.config*-Datei ändern, ist kein Neustart erforderlich, da App Service bei jeder Änderung von *web.config* einen automatischen Neustart durchführt.
...
## <a name="publish-to-app-service---visual-studio-2015"></a>Veröffentlichen in App Service: Visual Studio 2015
...
1. Wählen Sie nach Bedarf **Weiter** aus, um zusätzliche Einstellungen zu überprüfen. Wenn Sie planen, [Ihren Python-Code in Azure remote zu debuggen](debugging-remote-python-code-on-azure.md), müssen Sie für **Konfiguration** **Debuggen** festlegen.
...
## <a name="remote-debugging-on-azure-app-service"></a>Remotedebuggen auf Azure App Service

Wenn Sie eine **Debugkonfiguration** aus Visual Studio 2015 veröffentlichen, erstellt der Prozess automatisch eine *web.debug.config*-Datei und fügt einen *ptvsd*-Ordner hinzu, der die benötigten Tools zum Debuggen enthält.

Bei Visual Studio 2017 fügen Sie stattdessen diese Komponenten direkt aus dem Projekt hinzu. Klicken Sie mit der rechten Maustaste auf das Projekt im **Projektmappen-Explorer**, wählen Sie **Hinzufügen** > **Neues Element** und dann die Vorlage **„web.config“ für das Remotedebuggen in Azure** aus. Im Projekt wird eine *web.debug.config*-Datei zum Debuggen und ein *ptvsd*-Toolordner angezeigt.

Sobald diese Dateien auf dem Server bereitgestellt werden (dies geschieht bei Visual Studio 2015 automatisch und bei Visual Studio 2017 während des nächsten Veröffentlichungsprozesses), befolgen Sie die Anweisungen unter [Remotedebuggen von Python-Code in Azure](debugging-remote-python-code-on-azure.md).
