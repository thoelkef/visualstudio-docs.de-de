...
```  
 ...
 Ein expliziter Vergleich eignet sich zum Überprüfen auf bestimmte `HRESULT`-Werte, z. B. `E_FAIL`. Otherwise, use the `SUCCEEDED` or `FAILED` macros.  
