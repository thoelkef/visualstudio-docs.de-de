...
|[CA1802: Nach Möglichkeit Literale verwenden](../code-quality/ca1802-use-literals-where-appropriate.md)|Ein Feld wird als statisch und schreibgeschützt deklariert (in [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] freigegeben und schreibgeschützt) und mit einem Wert initialisiert, der zur Kompilierzeit berechnet werden kann. Da der Wert, der dem Zielfeld zugewiesen ist, zur Kompilierzeit komprimiert werden kann, ändern Sie die Deklaration in ein Konstantenfeld (Konstantenin [!INCLUDE[vbprvb](../includes/vbprvb-md.md)]), sodass der Wert zur Kompilierzeit anstelle der Laufzeit berechnet wird.|
...
|[CA1814: Verzweigte Arrays mehrdimensionalen Arrays vorziehen](../code-quality/ca1814-prefer-jagged-arrays-over-multidimensional.md)|Ein verzweigtes Array ist ein Array, dessen Elemente wiederum Arrays sind. Die Arrays, aus denen die Elemente bestehen, können unterschiedlich groß sein, was zu weniger Verlust von Speicherplatz für einige Datenmengen führen kann.|
...
|[CA1816: GC.SuppressFinalize korrekt aufrufen](../code-quality/ca1816-call-gc-suppressfinalize-correctly.md)|Eine Methode, bei der es sich um eine-Implementierung handelt, ruft keine GC auf. SuppressFinalize oder eine Methode, die keine Implementierung von verwerfen ist, ruft GC auf. SuppressFinalize oder eine Methode ruft GC auf. SuppressFinalize und übergibt etwas anderes als dieses (mich in [!INCLUDE[vbprvb](../includes/vbprvb-md.md)]).|
