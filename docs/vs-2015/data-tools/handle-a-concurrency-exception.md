...
## <a name="create-a-data-bound-datagridview-control"></a>Erstellen eines Daten gebundenen DataGridView-Steuer Elements
 In diesem Abschnitt erstellen Sie eine <xref:System.Windows.Forms.DataGridView>, indem Sie das Element **Customers** aus dem **Datenquellen** Fenster auf das Windows Form ziehen.
...
4. Ziehen Sie die Tabelle auf einen leeren Bereich des Formulars.
     ...
     Ein <xref:System.Windows.Forms.DataGridView>-Steuerelement mit dem Namen `CustomersDataGridView` und eine <xref:System.Windows.Forms.BindingNavigator> benannte `CustomersBindingNavigator` werden dem Formular hinzugefügt, das an den <xref:System.Windows.Forms.BindingSource> gebunden ist. dieses ist in, wird an die `Customers` Tabelle im `NorthwindDataSet` gebunden.
...
### <a name="addcode-to-handle-the-concurrency-exception"></a>Addcode zur Behandlung der Parallelitäts Ausnahme
 ...
 In diesem Abschnitt fügen Sie Code hinzu, mit dem versucht wird, die Datenbank zu aktualisieren. Außerdem werden alle <xref:System.Data.DBConcurrencyException>, die möglicherweise ausgelöst werden, sowie alle anderen Ausnahmen behandelt.
