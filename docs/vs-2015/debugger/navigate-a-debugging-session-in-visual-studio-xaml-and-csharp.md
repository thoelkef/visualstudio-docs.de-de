---
title: Navigieren in einer Debugsitzung (XAML und c#) | Microsoft-Dokumentation
ms.date: 11/15/2016
...
- [Festlegen eines bedingten halte Punkts, Ausführen bis zum Cursor und visualisieren einer Variablen](#BKMK_ConditionCursorVisualize)

- [Bearbeiten und Fortfahren, Wiederherstellung nach einer Ausnahme](#BKMK_EditContinueRecoverExceptions)

## <a name="create-the-sample-app"></a><a name="BKMK_CreateTheApplication"></a> Erstellen der Beispiel-App
 ...
 **Ersetzen Sie den MainPage-Konstruktor durch den Beispielcode.** Löschen Sie die MainPage()-Methode. Folgen Sie diesem Link: [Debugger-Navigations Beispielcode (XAML und c#)](../debugger/debugger-navigation-sample-code-xaml-and-csharp.md), und kopieren Sie dann den im c#-Abschnitt aufgeführten Code in die Zwischenablage. (Wählen Sie im Browser oder Help Viewer **zurück** , um zu dieser schnell Startseite zurückzukehren.) Fügen Sie im Visual Studio-Editor den Code in den- `partial class MainPage` Block ein. Wählen Sie STRG + S, um die Datei zu speichern.

 Sie können nun den Beispielen in diesem Thema folgen.

## <a name="set-and-run-to-a-breakpoint-step-into-a-method-and-examine-program-data"></a><a name="BKMK_StepInto"></a> Festlegen eines halte Punkts und Ausführen bis zu einem Breakpoint, schrittweises Ausführen einer Methode und untersuchen von Programm Daten
 ...
 **Legen Sie einen Breakpoint fest.** Legen Sie im MainPage-Konstruktor einen Haltepunkt in der `methodTrack = "Main Page";` -Anweisung fest. Wählen Sie die Zeile im schattierten Bundsteg des Quellcode-Editors aus (Tastatur: Positionieren Sie den Cursor in der Zeile, und drücken Sie F9).

 ![Einzelschritt](../debugger/media/dbg-basics-stepinto.png "DBG_Basics_StepInto")

 Das Haltepunktsymbol wird im Bundsteg angezeigt.

 **Ausführen bis zum Haltepunkt.** Starten Sie die Debugsitzung, indem Sie im Menü **Debuggen** die Option **Debugging starten** auswählen (Tastatur: F5).
...
- Wenn es sich bei der Anweisung um den Aufruf einer Funktion in der Projektmappe handelt, wechselt der Debugger in den Einstiegspunkt der aufgerufenen Funktion und hält dann die Ausführung an.
  ...
  **Überprüfen der Variablenwerte in den Datentipps.** Wenn Sie die Maus auf einen Variablennamen bewegen, werden der Name, der Wert und der Typ der Variablen in einem Datentipp angezeigt.
  ...
  ![Debuggerdatentipp](../debugger/media/dbg-basics-datatip.png "DBG_Basics_DataTip")
  ...
  **Hinzufügen einer Überwachung für die "methodTrack"-Variable.** Die `methodWatch` -Variable wird in diesem Schnellstart verwendet, um die in den Beispielen aufgerufenen Methoden aufzuzeigen. Um das Anzeigen des Variablenwerts zu vereinfachen, fügen Sie diesen einem Überwachungsfenster hinzu. Klicken Sie im Lokalfenster mit der rechten Maustaste auf den Variablennamen, und wählen Sie dann **Überwachung hinzufügen**aus.
  ...
  ![Überwachungsfenster](../debugger/media/dbg-basics-watchwindow.png "DBG_Basics_WatchWindow")
...
## <a name="step-into-over-and-out-of-methods"></a><a name="BKMK_StepIntoOverOut"></a> Einzel-und Prozedur Schritte für Methoden
 ...
 **Aufrufen der Methode "Example2" im MainPage-Konstruktor.** Bearbeiten Sie den MainPage-Konstruktor, und ersetzen Sie die Zeile nach `methodTrack = String.Empty;` durch `Example2();`aus.
 ...
 ![Example2-Methode aus der Demomethode aufrufen](../debugger/media/dbg-basics-callexample2.png "DBG_Basics_CallExample2")

 **Ausführen bis zum Haltepunkt.** Starten Sie die Debugsitzung, indem Sie im Menü **Debuggen** die Option **Debugging starten** auswählen (Tastatur: F5). Der Debugger unterbricht die Ausführung am Haltepunkt.
 ...
 **Debuggen.** Wählen Sie im Menü "Debuggen" die Option "Debuggen beenden" aus (Tastatur: UMSCHALT+F5). Damit wird die Debugsitzung beendet.

## <a name="set-a-conditional-breakpoint-run-to-the-cursor-and-visualize-a-variable"></a><a name="BKMK_ConditionCursorVisualize"></a> Festlegen eines bedingten Haltepunkts, Ausführen bis zum Cursor und Visualisieren einer Variablen
 ...
 **Aufrufen der Methode "Example3" im MainPage-Konstruktor.** Bearbeiten Sie den MainPage-Konstruktor, und ersetzen Sie die Zeile nach `methodTrack = String.Empty;` durch die Zeile `Example3();`aus.
 ...
 ![Example3-Methode aus der Demomethode aufrufen](../debugger/media/dbg-basics-callexample3.png "DBG_Basics_CallExample3")

 **Ausführen bis zum Haltepunkt.** Starten Sie die Debugsitzung, indem Sie im Menü **Debuggen** die Option **Debugging starten** auswählen (Tastatur: F5). Der Debugger unterbricht die Ausführung am Haltepunkt in der MainPage-Methode.
 ...
 **Legen Sie einen bedingten Haltepunkt fest.** Klicken Sie im linken Bundsteg des Codefensters mit der rechten Maustaste auf die Zeile `x += i;` , und wählen Sie dann **Bedingung**aus. Aktivieren Sie das Kontrollkästchen **Bedingung** , und geben Sie dann im Textfeld `i == 500;` ein. Wählen Sie die Option **Ist "True"** und anschließend **OK**aus. Mithilfe des Haltepunkts können Sie den Wert an der 500. Iteration der `for` -Schleife überprüfen.

 ![Dialogfeld „Bedingung für Haltepunkt“](../debugger/media/dbg-basics-breakpointcondition.png "DBG_Basics_BreakpointCondition")
...
## <a name="edit-and-continue-recover-from-an-exception"></a><a name="BKMK_EditContinueRecoverExceptions"></a> Bearbeiten und fortfahren, Wiederherstellung nach einer Ausnahme
 ...
 **Ändern der Programmlogik.** Es ist offensichtlich, dass der Fehler in der `if` -Bedingung liegt: Der Wert von `x` sollte geändert werden, wenn `x` gleich "0", nicht jedoch wenn `x` nicht gleich null ist. Wählen Sie **Unterbrechen** , um die Logik der Methode zu korrigieren. Beim Versuch, die Zeile zu bearbeiten, wird ein weiteres Dialogfeld angezeigt.
 ...
 ![Dialogfeld "Bearbeiten und Fortfahren"](../debugger/media/dbg-basics-editandcontinuedlg.png "DBG_Basics_EditAndContinueDlg")
 ...
 **Ändern des Variablenwerts.** Überprüfen Sie den Wert von `x` im Datentipp- oder Lokalfenster. Er ist weiterhin "0" (null). Wenn Sie versuchen, die Anweisung auszuführen, die die ursprüngliche Ausnahme verursacht hat, wird diese erneut ausgelöst. Sie können den Wert von `x`ändern. Doppelklicken Sie im Lokalfenster auf die Spalte **Wert** der Zeile **x** . Ändern Sie den Wert von "0" in "1".
 ...
 ![Wert im Lokalfenster bearbeiten](../debugger/media/dbg-basics-editandcontinuefix.png "DBG_Basics_EditAndContinueFix")
