...
- Ein SDK ist eine Auflistung von Dateien, die von Visual Studio als einzelnes Referenzelement behandelt werden. Im Dialogfeld **Verweis-Manager** werden alle sdgs aufgelistet, die für das Projekt relevant sind, das beim Anzeigen des Dialog Felds geöffnet ist. Wenn Sie einem Projekt ein SDK hinzufügen, können Sie auf den Inhalt dieses SDK über IntelliSense, **Toolbox**, Designer, den **Objektkatalog**, MSBuild, Bereitstellung, Debuggen und Verpacken zugreifen. Weitere Informationen zu SDKs finden Sie unter [Erstellen eines Software Development Kits](../extensibility/creating-a-software-development-kit.md).
...
|MSBuild verwendet automatisch die H- oder LIB-Dateien.|J|Die Datei *sdkname*.-Eigenschaften teilt Visual Studio mit, wie das Visual C++ Verzeichnis usw. für die automatische Verwendung von h-oder LIB-Dateien eingerichtet wird.|N||
...
|Sie können an einem Ökosystem von Partnern (benutzerdefinierte Galerien, Repositorys usw.) teilhaben.|–|Die verfügbaren Repositorys umfassen Visual Studio Gallery, Microsoft Download Center und [!INCLUDE[win8_appstore_long](../includes/win8-appstore-long-md.md)].|J||
