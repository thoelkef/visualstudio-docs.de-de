...
Das JavaScript IntelliSense-Erweiterbarkeits Feature ermöglicht Ihnen das Anpassen von IntelliSense-Ergebnissen im JavaScript-Editor für Bibliotheken von Drittanbietern. Dadurch können Entwickler, die diese Bibliotheken verwenden, die Möglichkeiten verbessern.
 ...
 ![Beispiel für Anweisungs Vervollständigung](../ide/media/js-intellisense-completion.png "js_intellisense_completion")

 Wenn Ihre Bibliothek Beschreibungen von Variablen, Funktionen und Objekten in JavaScript-Standardkommentar Tags (//) enthält, profitieren Sie standardmäßig standardmäßig von den IntelliSense-Erweiterbarkeits Features, die beschreibende Informationen in einem Popup Fenster bereitstellen, das wird rechts von Elementen in einer Vervollständigungsliste angezeigt, oder wenn Sie die öffnende Klammer in einem Funktions aufrufstyp eingeben. Die Kommentare im Popup Fenster enthalten die Beschreibung des Members. Das folgende Beispiel zeigt das Popup Feld für eine Vervollständigungsliste.

 ![Beispiel für ein Popup-Popup&#45;Fenster für Quick Infos](../ide/media/js-intellisense-quickinfo.png "js_intellisense_quickinfo")
...
## <a name="handling-intellisense-events"></a>Behandeln von IntelliSense-Ereignissen
 Mit dem Erweiterbarkeits Feature können Sie IntelliSense-Ergebnisse anpassen, indem Sie Ereignisse wie z. b. das `statementcompletion`-Ereignis des Sprachdienst `intellisense`-Objekts abonnieren. Das folgende Beispiel zeigt eine einfache Erweiterung, die vom Sprachdienst verwendet wird, um Member auszublenden, die mit einem Unterstrich der Anweisungs Vervollständigung beginnen. Dieser Code befindet sich in der Datei "underscorefilter. js" und befindet sich im Ordner "\\ \\*Visual Studio-Installationspfad*\javascript\references".
...
```
 ...
 Im vorangehenden Code überprüft die Erweiterung die [Eigenschaft TargetName](#TargetName) und die [Ziel Eigenschaft](#Target) des `statementcompletion` Ereignis Objekts, um Objekte wie `this` und `window` auszuschließen, und um sicherzustellen, dass eine gültige Anweisungs Vervollständigungsliste sein kann. identifizierten. Wenn eine Vervollständigungsliste identifiziert werden kann, aktualisiert die Erweiterung die Eigenschaften Sammlung der Anweisungs Vervollständigungs [Elemente](#Items) , indem Member herausgefiltert werden, die mit einem Unterstrich beginnen.

 Weitere Beispiele finden Sie im Ordner "\\ \\*Visual Studio-Installationspfad*\javascript\references". Die Datei "showplaincomments. js" in diesem Ordner enthält Beispiele für die Verwendung anderer Ereignisse, um standardmäßige IntelliSense-Unterstützung für standardmäßige JavaScript-Kommentar Tags (//) bereitzustellen. Wie bei "underscorefilter. js" ist "showplaincomments. js" bereits als funktionierende Erweiterung verfügbar, und Sie können die resultierenden IntelliSense-Informationen sehen, wenn Sie Kommentar Tags in Ihrem Code für Variablen, Funktionen und Objekte verwenden. Weitere Beispiele finden Sie unter [Code Beispiele](#CodeExamples).
...
> Wenn Sie die in Visual Studio enthaltenen Erweiterungs Dateien ändern, deaktivieren Sie möglicherweise JavaScript IntelliSense oder das von der Erweiterung unterstützte Feature.
 ...
 In Ihrem Erweiterungs Code können Sie Handler für die folgenden Ereignis Typen erstellen, indem Sie `addEventListener` verwenden:

- `statementcompletion`, wodurch ein Handler für ein Anweisungs Abschluss Ereignis hinzugefügt wird. Mit der Anweisungs Vervollständigung wird eine Liste von Membern für einen bestimmten Typ bereitgestellt, der angezeigt wird, nachdem Sie ein Sonderzeichen (z. b. einen Zeitraum (.)) oder eine Liste der Bezeichner angezeigt haben, die beim Eingeben von oder beim Drücken von STRG + J Der Handler empfängt ein Ereignis Objekt vom Typ `CompletionEvent`, das die folgenden Member unterstützt [: Items-Eigenschaft](#Items), [Ziel Eigenschaft](#Target), [TargetName-Eigenschaft](#TargetName)und Scope- [Eigenschaft](#Scope).

- `signaturehelp`, das einen Handler für IntelliSense-Parameter Info hinzufügt. Parameterinformationen gibt Aufschluss über die Anzahl, die Namen und die Typen von Parametern, die für eine Funktion erforderlich sind. Der Handler empfängt ein Ereignis Objekt vom Typ `SignatureHelpEvent`, das die folgenden Member unterstützt: [Ziel Eigenschaft](#Target), [Eigenschaften Objekt Eigenschaft](#ParentObject), [functioncomments-Eigenschaft](#FunctionComments), [functionhelp-Eigenschaft](#FunctionHelp).

- `statementcompletionhint`, wodurch ein Handler für IntelliSense-QuickInfo hinzugefügt wird. Das Popup Fenster Quick Info zeigt die gesamte Deklaration für Bezeichner in Ihrem Code. Der Handler empfängt ein Ereignis Objekt vom Typ `CompletionHintEvent`, das die folgenden Member unterstützt: [completionitem-Eigenschaft](#CompletionItem)und [symbolhelp-Eigenschaft](#SymbolHelp).
...
## <a name="intellisenseObject"></a> IntelliSense-Objekt
 In der folgenden Tabelle sind die Funktionen aufgeführt, die für das `intellisense`-Objekt verfügbar sind. Das `intellisense`-Objekt ist nur zur Entwurfszeit verfügbar.

|Funktion|Beschreibung|
|--------------|-----------------|
|`addEventListener(type, handler);`|Fügt einen Ereignishandler für ein IntelliSense-Ereignis hinzu.<br /><br /> `type` ist ein Zeichen folgen Wert. Gültige Werte sind u.a. `statementcompletion`, `signaturehelp` und `statementcompletionhint`.<br /><br /> `handler` ist eine Ereignishandlerfunktion, die ein Ereignis Objekt eines der folgenden Typen empfängt:<br /><br /> -    `CompletionEvent`, der für das `statementcompletion` Ereignis verwendet wird.<br />-    `SignatureHelpEvent`, der für das `signaturehelp` Ereignis verwendet wird.<br />-    `CompletionHintEvent`, der für das `statementcompletionhint` Ereignis verwendet wird.<br /><br /> Beispiele für die Verwendung dieser Funktion finden Sie unter [Code Beispiele](#CodeExamples).|
...
|`getFunctionComments(func);`|Gibt die Kommentare für eine angegebene Funktion zurück.<br /><br /> `func` gibt die Funktion an, für die Kommentare zurückgegeben werden.<br /><br /> Sie können den `func`-Parameter festlegen, indem Sie `completionItem.value` verwenden.<br /><br /> Das zurückgegebene `functionComments` Objekt enthält die folgenden Member: `above`, `inside` und `paramComment`. Weitere Informationen finden Sie unter der [Eigenschaft "functioncomments](#FunctionComments) ".<br /><br /> `getFunctionComments` können nur innerhalb eines der Ereignishandler aufgerufen werden, die von `addEventListener` registriert werden.<br /><br /> Ein Beispiel für die Verwendung dieser Funktion finden Sie unter \\ \\*Visual Studio-Installationspfad*\javascript\references\showplaincomments.js.|
...
|`nullWithCompletionsOf(value);`|Gibt einen speziellen NULL-Wert zurück, für den die Vervollständigungsliste durch das-Objekt bestimmt wird, das im `value`-Parameter übergeben wird.<br /><br /> `value` bestimmt die Vervollständigungsliste für den zurückgegebenen Wert. `value` kann ein beliebiger Typ sein.<br /><br /> Der NULL-Rückgabewert wird zur Entwurfszeit als NULL behandelt, aber die Vervollständigungsliste für den Rückgabewert ist identisch mit der Vervollständigungsliste für den `value` Parameter.<br /><br /> Eine Verwendung für diese Funktion besteht darin, IntelliSense für einen Rückgabewert bereitzustellen, wenn der Rückgabetyp zur Laufzeit vorhersagbar ist, der Rückgabewert jedoch zur Entwurfszeit `null` wird.|
|`redirectDefinition(func, definition);`|Weist IntelliSense an, die bereitgestellte Definitions Funktion anstelle der ursprünglichen Func-Funktion zu verwenden, wenn Parameter Hilfe oder **Gehe zu Definition** angefordert werden.<br /><br /> `func` die die Zielfunktion angibt.<br /><br /> `definition` gibt die Funktion an, die anstelle der Zielfunktion für Parameterinformationen und **Gehe zu Definition**verwendet werden soll.|
|`setCallContext(func, thisArg);`|Legt den Aufrufkontext (oder den Gültigkeitsbereich) für die angegebene Funktion fest.<br /><br /> `func` gibt die Funktion an, für die der Bereich festgelegt werden soll.<br /><br /> `thisArg` ist ein Objektliteral, auf das das `this`-Schlüsselwort verweisen kann, das den neuen Bereich für den Member angibt. Sie können Argumente einschließen, die an diesen Parameter übergeben werden, z. b. `intellisense.setCallContext(func, { thisArg: "", args: [23,2] });`<br /><br /> `setCallContext` bietet ein ähnliches Verhalten wie `Function.prototype.bind`, mit der Ausnahme, dass es nur für die IntelliSense-Unterstützung zur Entwurfszeit verwendet wird. Sie können `setCallContext` verwenden, um den Funktionsbereich festzulegen, wenn Sie einen Code Code simulieren müssen, der andernfalls nicht erreichbar ist, sodass der Funktions Aufrufpunkt beim Abrufen der Funktion den korrekten Bereich und die richtigen Argumente enthält.|
|`undefinedWithCompletionsOf(value);`|Gibt einen besonderen, nicht definierten Wert zurück, für den die Vervollständigungsliste durch das Objekt bestimmt wird, das im `value`-Parameter übergeben wird.<br /><br /> `value` bestimmt die Vervollständigungsliste für den zurückgegebenen Wert. `value` kann ein beliebiger Typ sein.<br /><br /> Der nicht definierte Rückgabewert wird zur Entwurfszeit als nicht definiert behandelt, aber die Vervollständigungsliste für den Rückgabewert ist identisch mit der Vervollständigungsliste für den `value` Parameter.<br /><br /> Eine Verwendung für diese Funktion besteht darin, IntelliSense für einen Rückgabewert bereitzustellen, wenn der Rückgabetyp zur Laufzeit vorhersagbar ist, aber der Rückgabewert zur Entwurfszeit nicht definiert ist.|
...
### <a name="CompletionItem"></a>completionitem (Eigenschaft)
 Gibt den Bezeichner zurück, der als Vervollständigungs Element bezeichnet wird, für den ein QuickInfo-Popup-Popup Element angefordert wird. Diese Eigenschaft ist für das `statementcompletionhint`-Ereignis Objekt und für die [Items-Eigenschaften](#Items) Eigenschaft des `statementcompletion`-Ereignis Objekts verfügbar.

 Rückgabewert: `completionItem` Objekt

 Im folgenden sind die Member des `completionItem` Objekts aufgeführt:

- `name` Lese-/Schreibzugriff bei Verwendung in der `items` Auflistung; Andernfalls schreibgeschützt. Gibt eine Zeichenfolge zurück, die das Vervollständigungs Element bezeichnet.

- `kind` Lese-/Schreibzugriff bei Verwendung in der `items` Auflistung; Andernfalls schreibgeschützt. Gibt eine Zeichenfolge zurück, die den Typ des Vervollständigungs Elements darstellt. Mögliche Werte sind Methode, Feld, Eigenschaft, Parameter, Variable und reserviert.

- `glyph` Lese-/Schreibzugriff bei Verwendung in der `items` Auflistung; Andernfalls schreibgeschützt. Gibt eine Zeichenfolge zurück, die ein Symbol darstellt, das in der Vervollständigungsliste angezeigt wird. Die möglichen Werte für `glyph` verwenden das folgende Format: vs:*GlyphType*, wobei *GlyphType* den sprachunabhängigen Membern in der <xref:Microsoft.VisualStudio.Language.Intellisense.StandardGlyphGroup>-Enumeration entspricht. Beispielsweise ist `vs:GlyphGroupMethod` ein möglicher Wert für `glyph`. Wenn `glyph` nicht festgelegt ist, bestimmt die `kind`-Eigenschaft das Standard Symbol.
...
### <a name="Items"></a>Items-Eigenschaft
...
- `signatures` Lese-/Schreibzugriff. Ruft das Array von Funktions Signaturen ab oder legt es fest. Jedes Element im Array ist ein `signature` Objekt. Einige `signature` Eigenschaften, z. b. `locid`, entsprechen allgemeinen [XML-Dokumentations Kommentar](../ide/xml-documentation-comments-javascript.md) Attributen.
  ...
  - `params` Lese-/Schreibzugriff. Ruft das Array von Parametern für die Funktion ab oder legt es fest. Jedes Element im Parameter Array ist ein `parameter` Objekt, das über Eigenschaften verfügt, die den folgenden Attributen des [\<param >](../ide/param-javascript.md) Elements entsprechen:
    ...
    - `optional` Lese-/Schreibzugriff. Gibt eine Zeichenfolge zurück, die angibt, ob der Parameter optional ist. `true` gibt an, dass der Parameter optional ist.  `false` bedeutet, dass dies nicht der Fall ist.

  - `returnValue` Lese-/Schreibzugriff. Ruft ein Rückgabewert Objekt mit Eigenschaften ab, die den folgenden Attributen des [\<returns >](../ide/returns-javascript.md) -Elements entsprechen, oder legt es fest:
...
### <a name="ParentObject"></a>para Object-Eigenschaft
 Gibt das übergeordnete Objekt einer Member-Funktion zurück. Beispielsweise gibt `parentObject` für `document.getElementByID` das `document`-Objekt zurück. Diese Eigenschaft ist für das `signaturehelp` Ereignis Objekt verfügbar.

 Rückgabewert: Objekt

### <a name="Target"></a> Zieleigenschaft
 Gibt ein Objekt zurück, das das Element auf der linken Seite des auslöserzeichens darstellt, bei dem es sich um einen Punkt (.) handelt. Bei Functions gibt `target` die Funktion zurück, für die Parameter Informationen angefordert werden. Diese Eigenschaft ist für die `statementcompletion`-und `signaturehelp`-Ereignis Objekte verfügbar.

 Rückgabewert: Objekt

### <a name="TargetName"></a>TargetName-Eigenschaft
 Gibt eine Zeichenfolge zurück, die das Ziel darstellt. Für "this." gibt `targetName` z. b. "This" zurück. Für "A. B" (wenn sich der Cursor hinter "b" befindet) gibt `targetName` "b" zurück. Diese Eigenschaft ist für das `statementcompletion` Ereignis Objekt verfügbar.
...
### <a name="SymbolHelp"></a>symbolhelp (Eigenschaft)
 ...
 Einige Eigenschaften des `symbolHelp` Objekts, z. b. `locid`, entsprechen allgemeinen [XML-Dokumentations Kommentar](../ide/xml-documentation-comments-javascript.md) Attributen.
...
- `symbolDisplayType` Lese-/Schreibzugriff. Gibt eine Zeichenfolge zurück, die den anzuzeigenden Typnamen enthält. Wenn `symbolDisplayType` nicht festgelegt ist, wird `symbolType` verwendet.

- `elementType` Lese-/Schreibzugriff. Wenn die `symbolType` `Array` ist, wird eine Zeichenfolge zurückgegeben, die den Typ der Elemente im Array darstellt.
...
- `functionHelp` Lese-/Schreibzugriff. Gibt eine [functionhelp-Eigenschaft](#FunctionHelp)zurück, die möglicherweise Informationen enthält, wenn das `symbolType` ist.
...
- showPlainComments.js. Dieser Code bietet IntelliSense-Unterstützung für Standard Kommentare. Sie enthält Ereignishandler für die Ereignisse `signaturehelp` und `statementcompletionhint`.
...
1. Fügen Sie den folgenden Code zu "DEMOLIB. js" hinzu.
...
2. Fügen Sie den folgenden Code zu "DEMOLIB. IntelliSense. js" hinzu.
...
### <a name="Logging"></a>Senden von Nachrichten an den Ausgabefenster
...
- examplelib. IntelliSense. js, die IntelliSense-Erweiterung. Diese Datei muss nicht in das Projekt aufgenommen werden, muss sich aber im gleichen Ordner wie "exampleLib.js" befinden.

- appCode.js, eine Projektdatei, die App-Code darstellt.

##### <a name="to-send-a-message-to-the-output-window"></a>So senden Sie eine Nachricht an das Ausgabefenster

1. Fügen Sie der Datei "examplelib. js" den folgenden Code hinzu.
...
2. Fügen Sie der Datei "examplelib. IntelliSense. js" den folgenden Code hinzu.
...
4. Geben Sie in appCode.js den folgenden Code ein. Wenn Sie eingeben, sehen Sie, dass sich das Symbol für den Namespace in "{}" geändert hat, wie in C#verwendet wird.

     ![Beispiel für die Verwendung der Symbol Eigenschaft](../ide/media/js-intellisense-glyph-namespace.png "js_intellisense_glyph_namespace")
...
5. Geben Sie in appCode.js den folgenden Code ein. Wenn Sie eingeben, wird ein neues enumerationssymbol für das Enum1-Element und ein neues Klassen Symbol für den SomeClass1-Member angezeigt.
     ...
     ![Beispiel für die Verwendung der Symbol-Eigenschaft](../ide/media/js-intellisense-glyph-class-enum.png "js_intellisense_glyph_class_enum")

### <a name="Overriding"></a>Vermeiden von Lauf Zeit Effekten in IntelliSense-Ergebnissen
...
1. Fügen Sie der Datei "examplelib. js" den folgenden Code hinzu.
    ...
    ```
     ...
     Im vorangehenden Code ignoriert die umschließende Funktion anfängliche Aufrufe, basierend auf dem Wert `count`, und gibt keine Ergebnisse zurück.
...
3. Geben Sie in appCode.js den folgenden Code ein. Die bezeichnerliste wird anstelle von IntelliSense angezeigt, da die umschließende Funktion niemals aufgerufen wird, was bedeutet, dass die `throttled` Funktion keine Ergebnisse zurückgibt.
     ...
     ![Beispiel für das Überschreiben von IntelliSense-Ergebnissen](../ide/media/js-intellisense-override.png "js_intellisense_override")

4. Fügen Sie der Datei "examplelib. IntelliSense. js" den folgenden Code hinzu. Dadurch wird das Entwurfszeit Verhalten geändert, sodass IntelliSense wie erwartet für die umschließende Funktion angezeigt wird.
