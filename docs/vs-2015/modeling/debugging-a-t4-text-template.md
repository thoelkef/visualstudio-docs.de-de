...
|-------------------|-----------------|--------------|  
|Fehler beim Laden der Basisklasse{0}"aus der die Transformationsklasse erbt Klasse.|Tritt auf, wenn Sie die Basisklasse nicht finden können, die im `inherits`-Parameter einer Vorlagendirektive angegeben ist. Die Meldung gibt die Zeilennummer der Vorlagenanweisung an.|Stellen Sie sicher, dass die angegebene Klasse vorhanden ist und die zugehörige Assembly in einer Assemblyanweisung angegeben ist.|  
...
|Der Parameter "{0}' ist in der Direktive bereits vorhanden. Das Parameterduplikat wird ignoriert.|Tritt auf, wenn ein Parameter mehr als einmal in einer Anweisung angegeben wird. Die Meldung enthält den Namen des Parameters und die Zeilennummer der Direktive.|Entfernen Sie die doppelte Parameterspezifikation.|  
