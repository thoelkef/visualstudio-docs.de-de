...
- Schreiben Sie die Methoden in einer separaten Assembly (Klassenbibliothek), und nennen Sie Sie aus ihrer Vorlage. Verwenden Sie die `<#@assembly#>`-Direktive, um die Assembly zu laden, und `<#@import#>`, um den Namespace Kontext festzulegen. Beachten Sie, dass Sie zum erneuten Erstellen der Assembly beim Debuggen möglicherweise [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] abbrechen und neu starten müssen. Weitere Informationen finden Sie unter [T4-Text Vorlagen Direktiven](../modeling/t4-text-template-directives.md).
...
### <a name="get-data-from-includevsprvsincludesvsprvs-mdmd"></a>Daten aus [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] erhalten
 Wenn Sie die in [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] bereitgestellten Dienste verwenden möchten, legen Sie das `hostSpecific` Attribut fest, und laden Sie die `EnvDTE` Assembly. Beispiel:
