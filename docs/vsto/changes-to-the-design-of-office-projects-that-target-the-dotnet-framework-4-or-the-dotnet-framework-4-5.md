...
- [Aktualisieren von Menü Band Anpassungen in Office-Projekten, die Sie zum .NET Framework 4 oder zum .NET Framework 4,5 migrieren](/visualstudio/vsto/update-ribbon-customizations-in-office-projects-to-migrate-to-dotnet-framework-4-or-4-5)
...
|Host Element Klassen, wie z `ThisWorkbook`. `Sheet`b. `ThisDocument`, *n*, und `ThisAddIn`, stellen keine `Dispose` Methode mehr bereit, die Sie überschreiben können.|Sie müssen den Code in der `Dispose`-Methodenüberschreibung in den `Shutdown`-Ereignishandler in der Hostelementklasse, z. B. `ThisAddIn_Shutdown`, verschieben und die `Dispose`-Methodenüberschreibung aus der Hostelementklasse entfernen.|
...
- [Neuerungen bei der Office-Entwicklung](https://msdn.microsoft.com/library/bf054af2-c896-4723-aa15-6381145b14bb)
