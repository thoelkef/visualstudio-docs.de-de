---
title: Beibehalten von dynamische Steuerelementen in Office-Dokumente
ms.custom: ''
ms.date: 02/02/2017
ms.technology:
- office-development
...
ms.locfileid: "34692794"
---
# <a name="persist-dynamic-controls-in-office-documents"></a>Beibehalten von dynamische Steuerelementen in Office-Dokumente
...
### <a name="re-create-dynamic-host-controls-when-documents-are-opened"></a>Neuerstellen Sie dynamischen Hoststeuerelementen beim Öffnen von Dokumenten  
...
### <a name="re-create-windows-forms-controls-when-documents-are-opened"></a>Erstellen Sie Windows Forms-Steuerelemente neu, beim Öffnen von Dokumenten  
...
###  <a name="removingActiveX"></a> ActiveX-Wrappern in einem Add-in entfernen  
...
#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>ActiveX-Wrapper zu entfernen, wenn das Dokument geöffnet wird  
 Um alle ActiveX-Wrapper zu entfernen, rufen Sie die `GetVstoObject` Methode zum Generieren ein Hostelement für die <xref:Microsoft.Office.Interop.Word.Document> oder <xref:Microsoft.Office.Interop.Excel.Workbook> , das das neu geöffnete Dokument darstellt. Beispielsweise, um alle ActiveX-Wrapper aus einem Word-Dokument zu entfernen, können rufen Sie die `GetVstoObject` Methode zum Generieren ein Hostelement für die <xref:Microsoft.Office.Interop.Word.Document> -Objekt, das an den Ereignishandler für die <xref:Microsoft.Office.Interop.Word.ApplicationEvents4_Event.DocumentOpen> Ereignis.  
 ...
 Im folgenden Codebeispiel wird veranschaulicht, wie zum Aufrufen der `GetVstoObject` -Methode auf, wenn das Dokument geöffnet wird.  
  
 [!code-vb[Trin_WordAddInDynamicControls#11](../vsto/codesnippet/VisualBasic/trin_wordaddindynamiccontrols/ThisAddIn.vb#11)]
 [!code-csharp[Trin_WordAddInDynamicControls#11](../vsto/codesnippet/CSharp/Trin_WordAddInDynamicControls/ThisAddIn.cs#11)]  
  
 Obwohl die `GetVstoObject` Methode wird in erster Linie zum Generieren eines neuen Hostelements zur Laufzeit verwendet, diese Methode löscht auch alle ActiveX-Wrapper aus dem Dokument das erste Mal für ein bestimmtes Dokument aufgerufen wird. Weitere Informationen zur Verwendung der `GetVstoObject` -Methode finden Sie unter [Erweitern von Word-Dokumenten und Excel-Arbeitsmappen in VSTO-Add-ins zur Laufzeit](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md).  
  
 Beachten Sie, wenn Ihr VSTO-Add-in beim Öffnen des Dokuments dynamische Steuerelemente erstellt, das VSTO-Add-in bereits Ruft die `GetVstoObject` -Methode im Rahmen des Prozesses, der die Steuerelemente zu erstellen. Sie müssen nicht auf einen separaten Aufruf zum Hinzufügen der `GetVstoObject` Methode, um in diesem Szenario die ActiveX-Wrapper zu entfernen.  
