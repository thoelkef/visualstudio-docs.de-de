...
dev_langs: 
  ...
  - "CSharp"
helpviewer_keywords: 
  - "Problembehandlung [Office-Entwicklung in Visual Studio], Sicherheit"
  - "Vertrauenswürdiger Code [Office-Entwicklung in Visual Studio]"
  - "Blockierter Code [Office-Entwicklung in Visual Studio]"
  - "Outlook [Office-Entwicklung in Visual Studio], Objektmodellschutz"
  - "Bösartiger Code [Office-Entwicklung in Visual Studio]"
  - "Outlook-Objektmodellschutz [Office-Entwicklung in Visual Studio]"
...
### Minimieren der Warnungen vom Objektmodellschutz  
 ...
 Im folgenden Codebeispiel wird veranschaulicht, wie die beschränkte To\-Eigenschaft eines Microsoft.Office.Interop.Outlook.MailItem\-Objekts verwendet werden kann, das für den Objektmodellschutz vertrauenswürdig ist. Im Code wird das vertrauenswürdige `Application`\-Feld verwendet, um das Microsoft.Office.Interop.Outlook.MailItem\-Objekt abzurufen.  
...
> [!NOTE]  
>  Wenn Outlook mit Exchange verwendet wird, ist es nicht möglich, durch ein Abrufen aller Outlook\-Objekte aus `ThisAddIn.Application` sicherzustellen, dass Ihr VSTO\-Add\-In auf das gesamte Outlook\-Objektmodell zugreifen kann. Wenn beispielsweise ein Exchange\-Administrator Outlook so einstellt, dass automatisch Versuche abgelehnt werden, über das Outlook\-Objektmodell auf Adressinformationen zuzugreifen, verhindert Outlook für das vorherige Codebeispiel den Zugriff auf die To\-Eigenschaft, obwohl im Codebeispiel das vertrauenswürdige `ThisAddIn.Application`.Application\-Feld verwendet wird.  
...
#### So deaktivieren Sie VSTO\-Add\-Ins in Microsoft Office 2010\- und Microsoft [!INCLUDE[Office_15_short](../vsto/includes/office-15-short-md.md)]\-Anwendungen  
