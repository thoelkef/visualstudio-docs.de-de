...
## Parameter  
 ...
 `dateVal`  
 Erforderlich.  Als numerischer Wert gibt `dateVal` die Anzahl der Millisekunden in UTC \(Universal Time Coordinated\) zwischen dem angegebenen Datum und Mitternacht des 1. Januar 1970 an.  Als Zeichenfolge wird `dateVal` entsprechend den Regeln in [Datum\- und Uhrzeitzeichenfolgen](../../javascript/date-and-time-strings-javascript.md) analysiert.  Das `dateVal`\-Argument kann auch ein VT\_DATE\-Wert sein, der von einigen ActiveX\-Objekten zurückgegeben wird.  
...
## Hinweise  
 ...
 Weitere Informationen zur Verwendung des `Date`\-Objekts und verwandter Methoden finden Sie unter [Berechnen von Datum und Uhrzeit \(JavaScript\)](../../javascript/calculating-dates-and-times-javascript.md).  
...
|[getVarDate\-Methode](../../javascript/reference/getvardate-method-date-javascript.md)|Gibt den `VT_DATE`\-Wert in einem `Date`\-Objekt zurück.|  
...
|[toJSON\-Methode](../../javascript/reference/tojson-method-date-javascript.md)|Wird verwendet, um Daten eines Objekttyps vor der JSON\-Serialisierung umzuwandeln.|  
